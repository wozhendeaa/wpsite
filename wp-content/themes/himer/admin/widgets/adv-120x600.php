<?php
/* Adv 120x600 */
add_action( 'widgets_init', 'himer_widget_adv120x600_widget' );
function himer_widget_adv120x600_widget() {
...
class Widget_Adv120x600 extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'adv120x600-widget',himer_theme_name.' - Adv 120x600', $widget_ops, $control_ops );
	}
		...
		if ($show_ads == true) {
			...
			echo stripcslashes($before_widget);
				if ($title) {
					...
					echo "<h3 class='screen-reader-text'>".esc_html__("Adv 120x600","himer")."</h3>";
				}?>
