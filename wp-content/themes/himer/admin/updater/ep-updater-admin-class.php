...
if ( ! class_exists( 'ElitePack_Theme_Updater_Admin' ) ) {
	...
	class ElitePack_Theme_Updater_Admin {
			...
			if( ! empty( $new_notice ) ){
				...
				<script type="text/javascript">
					jQuery(window).load(function() {
						var $theme = jQuery('*[data-slug="<?php echo stripcslashes($this->slug) ?>"]').find('.update-message p');
						if( typeof $theme != 'undefined') {
							$theme.append( ' <strong><?php echo stripcslashes($new_notice); ?></strong>' );
						}
					});
				</script>
				<?php
		...
		function insert_deactivation_form(){
			...
			<div id="<?php echo esc_attr($modal_id) ?>" class="ep-modal <?php echo "{$this->prefix}-ep-modal" ?>">
				<div class="ep-modal-header">
					<div>
						<h2><?php printf( $this->strings('feedback'), $this->name ); ?></h2>
					</div>
					<button class="ep-modal-close <?php echo esc_attr($modal_id) ?>-cancel dashicons dashicons-no"><span class="screen-reader-text"><?php echo stripcslashes($this->strings('cancel')); ?></span></button>
				</div>
				<div class="ep-modal-content">
					<h3><?php echo stripcslashes($this->strings('deactivation-share-reason')); ?></h3>
					<ul>
						<?php
							...
							foreach ( $reasons as $reason_id => $the_reason ) {
								...
								<li>
									...
									<input type="radio" class="<?php echo esc_attr($modal_id) ?>-reason" name="<?php echo esc_attr($modal_id) ?>-reason" id="<?php echo esc_attr( $modal_id .'-reason-'. $reason_id ) ?>" value="<?php echo esc_attr( $reason_id ) ?>">
									<label for="<?php echo esc_attr( $modal_id .'-reason-'. $reason_id ) ?>"><?php echo esc_html($the_reason['title']); ?></label>
					...
					<input id="<?php echo esc_attr($modal_id) ?>-the-reason" type="hidden" value="">
					<input id="<?php echo esc_attr($modal_id) ?>-the-details" type="hidden" value="">
				</div>
				<div class="ep-modal-footer">
					<div>
						<a id="<?php echo esc_attr($modal_id) ?>-send-deactivation" href="#" class="button button-primary ep-modal-disabled ep-send-deactivation" disabled><?php echo stripcslashes($this->strings('send')); ?></a>
						<button class="button button-secondary <?php echo esc_attr($modal_id) ?>-cancel"><?php echo stripcslashes($this->strings('cancel')); ?></button>
					</div>
					<a href="#" id="<?php echo esc_attr($modal_id) ?>-deactivation-skip" class="button button-secondary ep-deactivate-skip"><?php echo stripcslashes($this->strings('skip')); ?></a>
				</div>
			</div>

			<div id="<?php echo esc_attr($modal_id) ?>-overlay" class="ep-mpdal-overlay"></div>
			<?php
		}
