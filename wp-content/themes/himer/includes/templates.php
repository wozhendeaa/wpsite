<?php if (isset($blog_h) && $blog_h == "blog_h") {
	...
	$orderby_post            = himer_options("orderby_post_h");
	$order_post              = himer_options("order_post_h");
	$post_display            = himer_options("post_display_h");
	$post_single_category    = himer_options("post_single_category_h");
	$post_categories         = himer_options("post_categories_h");
	$post_exclude_categories = himer_options("post_exclude_categories_h");
	$post_posts              = himer_options("post_posts_h");
	...
	if (isset($first_one) && $first_one != "") {
		if ((isset($tab_category) && $tab_category == true) || (isset($tab_tag) && $tab_tag == true)) {
			...
			if ($first_one == $recent_questions_slug_2) {
				$specific_date = himer_options("date_recent_questions".$is_tag_dash);
			}else if ($first_one == $most_answers_slug_2) {
				$specific_date = himer_options("date_most_answered".$is_tag_dash);
			}else if ($first_one == $no_answers_slug_2) {
				$specific_date = himer_options("date_no_answers".$is_tag_dash);
			}else if ($first_one == $most_visit_slug_2) {
				$specific_date = himer_options("date_most_visited".$is_tag_dash);
			}else if ($first_one == $most_reacted_slug_2) {
				$specific_date = himer_options("date_most_reacted".$is_tag_dash);
			}else if ($first_one == $most_vote_slug_2) {
				$specific_date = himer_options("date_most_voted".$is_tag_dash);
			}else if ($first_one == $random_slug_2) {
				$specific_date = himer_options("date_random_questions".$is_tag_dash);
			}else if ($first_one == $question_new_slug_2) {
				$specific_date = himer_options("date_new_questions".$is_tag_dash);
			}else if ($first_one == $question_sticky_slug_2) {
				$specific_date = himer_options("date_sticky_questions".$is_tag_dash);
			}else if ($first_one == $question_polls_slug_2) {
				$specific_date = himer_options("date_poll_questions".$is_tag_dash);
			}else if ($first_one == $question_followed_slug_2) {
				$specific_date = himer_options("date_followed_questions".$is_tag_dash);
			}else if ($first_one == $question_favorites_slug_2) {
				$specific_date = himer_options("date_favorites_questions".$is_tag_dash);
			}else if ($first_one == $recent_posts_slug_2) {
				$specific_date = himer_options("date_recent_posts".$is_tag_dash);
			}else if ($first_one == $posts_visited_slug_2) {
				$specific_date = himer_options("date_posts_visited".$is_tag_dash);
			}else if ($first_one == $question_bump_slug_2) {
				$specific_date = himer_options("date_question_bump".$is_tag_dash);
			}else if ($first_one == $answers_slug_2) {
				$specific_date = himer_options("date_answers".$is_tag_dash);
			}
			$orderby_answers = himer_options("orderby_answers".$is_tag_dash);
		}else {
			if ($first_one == $feed_slug_2) {
				$specific_date = himer_post_meta("date_feed");
			}else if ($first_one == $recent_questions_slug_2) {
				$specific_date = himer_post_meta("date_recent_questions");
			}else if ($first_one == $questions_for_you_slug_2) {
				$specific_date = himer_post_meta("date_questions_for_you");
			}else if ($first_one == $most_answers_slug_2) {
				$specific_date = himer_post_meta("date_most_answered");
			}else if ($first_one == $no_answers_slug_2) {
				$specific_date = himer_post_meta("date_no_answers");
			}else if ($first_one == $most_visit_slug_2) {
				$specific_date = himer_post_meta("date_most_visited");
			}else if ($first_one == $most_reacted_slug_2) {
				$specific_date = himer_post_meta("date_most_reacted");
			}else if ($first_one == $most_vote_slug_2) {
				$specific_date = himer_post_meta("date_most_voted");
			}else if ($first_one == $random_slug_2) {
				$specific_date = himer_post_meta("date_random_questions");
			}else if ($first_one == $question_new_slug_2) {
				$specific_date = himer_post_meta("date_new_questions");
			}else if ($first_one == $question_sticky_slug_2) {
				$specific_date = himer_post_meta("date_sticky_questions");
			}else if ($first_one == $question_polls_slug_2) {
				$specific_date = himer_post_meta("date_poll_questions");
			}else if ($first_one == $question_followed_slug_2) {
				$specific_date = himer_post_meta("date_followed_questions");
			}else if ($first_one == $question_favorites_slug_2) {
				$specific_date = himer_post_meta("date_favorites_questions");
			}else if ($first_one == $recent_posts_slug_2) {
				$specific_date = himer_post_meta("date_recent_posts");
			}else if ($first_one == $posts_visited_slug_2) {
				$specific_date = himer_post_meta("date_posts_visited");
			}else if ($first_one == $question_bump_slug_2) {
				$specific_date = himer_post_meta("date_question_bump");
			}else if ($first_one == $answers_slug_2) {
				$specific_date = himer_post_meta("date_answers");
			}else if ($first_one == $answers_might_like_slug_2) {
				$specific_date = himer_post_meta("date_answers_might_like");
			}else if ($first_one == $answers_for_you_slug_2) {
				$specific_date = himer_post_meta("date_answers_for_you");
			}else if ($first_one == $poll_feed_slug_2) {
				$specific_date = himer_post_meta("date_poll_feed");
			}
		}
	}
	if (isset($wp_page_template) && $wp_page_template == "template-home.php") {
		$post_number             = himer_post_meta("posts_per_page");
		$order_post              = himer_post_meta("order_page_h");
		$orderby_answers         = himer_post_meta("orderby_answers_h");
		$active_points           = himer_options("active_points");
		$question_bump           = himer_options("question_bump");
		$post_display            = himer_post_meta("question_display_r");
		$post_single_category    = himer_post_meta("question_single_category_r");
		$post_categories         = himer_post_meta("question_categories_r");
		$post_exclude_categories = himer_post_meta("question_exclude_categories_r");
		$post_posts              = himer_post_meta("question_questions_r");
	}else {
		$order_post = "DESC";
	}
	if (isset($first_one) && $first_one != "" && ($first_one == "all" || $first_one == "q-0" || $first_one == $feed_slug || $first_one == $recent_questions_slug || $first_one == $questions_for_you_slug || $first_one == $answers_slug || $first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $most_answers_slug || $first_one == $no_answers_slug || $first_one == $most_visit_slug || $first_one == $most_reacted_slug || $first_one == $most_vote_slug || $first_one == $random_slug || $first_one == $question_new_slug || $first_one == $question_sticky_slug || $first_one == $question_polls_slug || $first_one == $question_followed_slug || $first_one == $question_favorites_slug || ($question_bump == "on" && $active_points == "on" && ($first_one == $question_bump_slug || $first_one == $question_bump_slug_2)) || $first_one == $poll_feed_slug || $first_one == $feed_slug_2 || $first_one == $recent_questions_slug_2 || $first_one == $questions_for_you_slug_2 || $first_one == $answers_slug_2 || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2 || $first_one == $most_answers_slug_2 || $first_one == $no_answers_slug_2 || $first_one == $most_visit_slug_2 || $first_one == $most_reacted_slug_2 || $first_one == $most_vote_slug_2 || $first_one == $random_slug_2 || $first_one == $question_new_slug_2 || $first_one == $question_sticky_slug_2 || $first_one == $question_polls_slug_2 || $first_one == $question_followed_slug_2 || $first_one == $question_favorites_slug_2 || $first_one == $poll_feed_slug_2)) {
		...
		$taxonomy  = 'category';
		$post_type = apply_filters('himer_recent_posts_post_type','post');
	}
}else if (isset($wp_page_template) && $wp_page_template == "template-question.php") {
	$taxonomy                = wpqa_question_categories;
	$post_type               = wpqa_questions_type;
	$orderby_post            = himer_post_meta("orderby_question_q");
	$order_post              = himer_post_meta("order_question");
	$post_display            = himer_post_meta("question_display_q");
	$post_single_category    = himer_post_meta("question_single_category_q");
	$post_categories         = himer_post_meta("question_categories_q");
	$post_exclude_categories = himer_post_meta("question_exclude_categories_q");
	$post_posts              = himer_post_meta("question_questions_q");
	$specific_date           = himer_post_meta("specific_date_q");
}else if (isset($wp_page_template) && $wp_page_template == "template-comments.php") {
	$comment_type    = himer_post_meta("comment_type");
	$orderby_answers = himer_post_meta("orderby_answers_a");
	$order_post      = himer_post_meta("order_answers");
	$custom_answers  = himer_post_meta("custom_answers");
	$specific_date   = himer_post_meta("specific_date_c");
	$post_pagination = himer_options("question_pagination");
	if ($custom_answers == "on") {
		$post_number     = himer_post_meta("answers_number");
		$post_pagination = himer_post_meta("answers_pagination");
	}
	...
	$taxonomy                = 'category';
	$post_type               = 'post';
	$orderby_post            = himer_post_meta("orderby_post_b");
	$order_post              = himer_post_meta("order_post");
	$post_display            = himer_post_meta("post_display_b");
	$post_single_category    = himer_post_meta("post_single_category_b");
	$post_categories         = himer_post_meta("post_categories_b");
	$post_exclude_categories = himer_post_meta("post_exclude_categories_b");
	$post_posts              = himer_post_meta("post_posts_b");
	$specific_date           = himer_post_meta("specific_date_b");
}
...
$theme_sidebar = $theme_sidebar_all = himer_post_meta("sidebar");
if ($theme_sidebar == "default") {
	$theme_sidebar = $theme_sidebar_all = himer_options("sidebar_layout");
}else {
