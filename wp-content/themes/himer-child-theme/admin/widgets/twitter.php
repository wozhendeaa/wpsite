<?php
/* Twitter */
add_action( 'widgets_init', 'himer_latest_tweet_widget' );
function himer_latest_tweet_widget() {
...
class Latest_Tweets extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'twitter-widget',himer_theme_name.' - Twitter', $widget_ops, $control_ops );
	}
		...
		if (has_wpqa() && wpqa_plugin_version >= "5.8") {
			echo stripcslashes($before_widget);
				if ($title) {
					...
					echo "<h3 class='screen-reader-text'>".esc_html__("Twitter","himer")."</h3>";
				}?>
					...
					if (isset($tweets) && is_array($tweets)) {
						...
						<ul>
							<?php foreach ($tweets as $item) {
								...
								$h_time = sprintf(esc_html__('about %s ago','himer'),human_time_diff($time));
								
								echo '<li class="tweet-item d-flex">
									<a target="_blank" href="'.esc_url($permalink).'" class="tweet-icon mr-3"><i class="icon-social-twitter"></i></a>
