<?php
/* Adv 234x60 */
add_action( 'widgets_init', 'himer_widget_adv234x60_widget' );
function himer_widget_adv234x60_widget() {
...
class Widget_Adv234x60 extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'adv234x60-widget',himer_theme_name.' - Adv 234x60', $widget_ops, $control_ops );
	}
		...
		if ($show_ads == true) {
			...
			echo stripcslashes($before_widget);
				if ($title) {
					...
					echo "<h3 class='screen-reader-text'>".esc_html__("Adv 234x60","himer")."</h3>";
				}?>
