    ...
    <table class="wpf-addon-table">
        <tr>
            <td>
                ...
                <p style="font-size: 20px;margin: 5px 0 10px 0px;"><?php _e('Member Search Form', 'wpforo-ucf'); ?></p>
                <p style="font-size: 14px;margin: 0px 0 20px 0px; font-family:Arial; margin-bottom:0px;"><?php _e('This is the structure of forum Member Search Form. Below you can see "row panel(s)". By default the Member Search Form consists of one row with two columns of fields.', 'wpforo-ucf'); ?></p>
                <ul style="margin-top:0px; list-style:circle; margin-left:30px; padding-top:5px; margin-bottom:20px;">
                    ...
                    <li style="margin-bottom: 1px;"><?php _e('To add a new custom field in form, just drag and drop fields from Inactive Fields area below to Member Search Form area.', 'wpforo-ucf'); ?></li>
                ...
                <div class="wpfucf-rows-wrapper wpfucf-sortable wpfucf-tab-search">
                    <?php
        ...
        <tr>
            <td style="text-align:right;">
                <?php $adminPostUrl = admin_url('admin-post.php?action=wpfucfResetSearchFields'); ?>
                <a href="<?php echo wp_nonce_url($adminPostUrl, 'wpfucfResetSearchFields'); ?>" class="button button-secondary wpfucf-reset-options" style="margin-left: 5px;float: left;">
                ...
                <input type="hidden" name="wpfucf-redirect-to" value="<?php echo admin_url('admin.php?page=' . self::UCF_PAGE_MAIN . '&tab=' . self::TAB_SEARCH); ?>">
            </td>
        </tr>
    </table>
    <div class="wpfucf-clear"></div>
...
<p style="font-size: 14px;margin: 0px 0 0 15px; font-family:Arial"><?php _e('To remove custom fields, just drag and drop fields from Member Search Form above to Inactive Fields area below.', 'wpforo-ucf'); ?></p>
<?php if (WPF()->settings->members['search_type'] === 'search') { ?>
    <div class="wpfucf-info">
        <h2><?php _e('Member search type is : search, which means all fields will be grouped into one!', 'wpforo-ucf'); ?></h2>
    </div>
<?php } ?>
            ...
            <div class="wpfucf-row-col">
                <div class="wpfucf-inactive-fields wpfucf-row-col-children" style="min-height:100px;overflow:hidden;">
                    ...
                    foreach ($fields as $field) {
                        $fKey = $field['fieldKey'];
                        $keys = WpforoUcfHelper::arraySearchByValue($searchFields, $fKey);
                        if (!$keys && WPF()->form->can_show_value($field)) {
                            $fieldObject = WpforoUcfHelper::getFieldObject($field);
                            if ($fieldObject->isSearchable($field)) {
