...
class MOBILE_API_Response {
	...
	function setup() {
		...
		if ($mobile_api->query->include) {
			$this->include_values = explode(',', $mobile_api->query->include);
		}
		// Props to ikesyo for submitting a fix!
		if ($mobile_api->query->exclude) {
			$this->exclude_values = explode(',', $mobile_api->query->exclude);
	...
	function get_json($data, $status = true) {
		global $mobile_api;
		// Include a status value with the response
		if (is_array($data)) {
			if (isset($data["status"]) && $data["status"] === "removestatus") {
				unset($data["status"]);
			}else {
		...
		$data = apply_filters('mobile_api_encode', $data);
		...
		if (function_exists('json_encode')) {
			...
			if (version_compare(PHP_VERSION, '5.3') < 0) {
				...
				if ($mobile_api->query->json_encode_options) {
					$json_encode_options = $mobile_api->query->json_encode_options;
				}
				$json = json_encode($data, $json_encode_options);
			}
		} else {
			// Use PEAR's Mobile_API_JSON encoder otherwise
			if (!class_exists('Mobile_API_JSON')) {
				$dir = mobile_api_dir();
				require_once "$dir/library/JSON.php";
			}
			$json_service = new Mobile_API_JSON();
			$json = $json_service->encode($data);
		}
		
		// Thanks to Stack Overflow user Gumbo stackoverflow.com/questions/2934563
		if ($mobile_api->query->json_unescaped_unicode) {
	...
	function respond($result, $status = true) {
		global $mobile_api;
		$json = $this->get_json($result, $status);
		$status_redirect = "redirect_$status";
		if ($mobile_api->query->dev || !empty($_REQUEST['dev'])) {
		...
		} else if ($mobile_api->query->redirect) {
			$url = $this->add_status_query_var($mobile_api->query->redirect, $status);
			wp_redirect($url);
		} else if ($mobile_api->query->callback) {
			// Run a JSONP-style callback with the result
			$this->callback($mobile_api->query->callback, $json);
		} else {
			// Output the result
			$this->output($json);
		}
