...
class MOBILE_API_Query {
	...
	function get_legacy_controller($json) {
		...
		if ($json == 'submit_comment') {
			if ($mobile_api->controller_is_active('comment')) {
				return 'comment';
			} else {
				$mobile_api->error(esc_html__("The submit_comment method has been removed from the Core controller. To use this method you must enable the Comment controller from WP Admin > Settings > JSON API.","mobile-api"));
			}
		} else if ($json == 'create_post') {
			if ($mobile_api->controller_is_active('posts')) {
				return 'posts';
			} else {
				$mobile_api->error(esc_html__("The create_post method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > JSON API.","mobile-api"));
			}
		} else {
			return 'core';
		}
	...
	function get_method($controller) {
		...
		if (empty($method)) {
			...
			return false;
		} else if (method_exists("MOBILE_API_{$controller}_Controller", $method)) {
			// Case 2: an explicit method was specified
			return $method;
		} else if (isset($_GET['json']) && $_GET['json'] == "tabs") {
			return 'tabs';
		} else if (isset($_GET['json']) && $_GET['json'] == "gettabscontent") {
			return 'get_tabs_content';
			...
			if (is_search()) {
				...
				if (empty($_GET['json'])) {
					$mobile_api->error(sprintf(__("Uknown method %s.","mobile-api"),$method));
				}
