...
function wpqa_group() {
	...
	if ($active_groups == "on") {
		...
		$out_payment = '<div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("Please make a payment to be able to create a group.","wpqa").'</p></div>';
		...
		if ($group_payment_style == "packages") {
			...
			$out_payment .= '<a href="'.wpqa_checkout_link("add_group").'" class="button-default btn btn__primary" target="_blank">'.esc_html__("Pay to create a group","wpqa").'</a>';
		}

		if (!is_user_logged_in() && $pay_group == "on" && $unlogged_pay != "on") {
			if ($activate_login != 'disabled') {
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i>'.esc_html__("You must login to create group.","wpqa").'</p></div>
				'.do_shortcode("[wpqa_login".(isset($a["popup"]) && $a["popup"] == "popup" && has_himer() || has_knowly()?" register='button'":"")."]");
			}else {
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to add group","wpqa").'.</p></div>';
			}
		}else {
			...
			if ($allow_to_add_group == true && !is_super_admin($user_id) && isset($_allow_to_group) && (int)$_allow_to_group < 1 && $pay_group == "on" && ($custom_permission != "on" || ($custom_permission == "on" && empty($roles["add_group_payment"])))) {
				...
				if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_id) && empty($roles["add_group"])) || (!is_user_logged_in() && $add_group != "on"))) {
					$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to add group","wpqa").(!is_user_logged_in()?", ".esc_html__("You must login to add group","wpqa"):'').'.'.' '.wpqa_paid_subscriptions().'</p></div>';
...
function wpqa_edit_group_attr() {
	...
	if (!is_user_logged_in()) {
		...
		if ($activate_login != 'disabled') {
			...
			$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to edit group.","wpqa").'</p></div>';
		}
	}else {
