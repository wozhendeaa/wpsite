...
if (!function_exists('wpqa_select_categories')) :
	function wpqa_select_categories($rand,$select,$attr = array(),$post_id = '',$taxonomy = '',$name = 'category',$show_option_none = '') {
		...
		if ($category_single_multi == "ajax" && $taxonomy != "category") {
			...
			$main_terms = $terms = $last_terms = array();
			...
			if (is_array($terms) && !empty($terms)) {
				$main_terms = $terms = array_unique($terms);
				foreach ($terms as $key => $value) {
					$parent = get_term($value,$taxonomy);
					if ($parent->parent == 0) {
						$last_terms[] = $value;
						unset($terms[$key]);
					}
			...
			if (is_array($terms) && !empty($terms)) {
				$last_terms = wpqa_get_next_child_category($terms,$taxonomy,$last_terms,$main_terms);
			}
			$terms = (is_array($last_terms) && !empty($last_terms)?$last_terms:$terms);
			...
			$args = array_merge($exclude,array(
				...
				'taxonomy'      => $taxonomy,
				'id'            => ($taxonomy == wpqa_question_categories?wpqa_question_categories:"post-category").'-'.$rand,
				'name'          => $name
			));
			return '<ul class="row row-boot row-warp">'.wpqa_categories_checklist($args).'</ul>';
		}else {
			...
			$dropdown_categories = wp_dropdown_categories(array_merge($exclude,$parent,array("orderby" => "name","echo" => "0","show_option_none" => $show_option_none,'taxonomy' => $taxonomy, 'hide_empty' => 0,'depth' => 0,'class' => "form-control wpqa-custom-select",'id' => ($taxonomy == wpqa_question_categories?wpqa_question_categories:"post-category").'-'.$rand,'name' => $name,'hierarchical' => true,'selected' => $select)));
			return '<span class="styled-select">'.$dropdown_categories.'</span>';
		}
	}
endif;
/* Get next child */
function wpqa_get_next_child_category($terms,$taxonomy,$last_terms,$main_terms) {
	if (is_array($terms) && !empty($terms)) {
		$child_c = get_terms(array('taxonomy' => $taxonomy,'parent' => end($last_terms),'hide_empty' => 0));
		if (!is_wp_error($child_c) && !empty($child_c)) {
			foreach ($child_c as $value) {
				if ($value->parent > 0 && $value->parent == end($last_terms) && in_array($value->term_id,$main_terms)) {
					$last_terms[] = $value->term_id;
					$terms = array_diff($terms,array($value->term_id));
					if (is_array($terms) && !empty($terms)) {
						$last_terms = wpqa_get_next_child_category($terms,$taxonomy,$last_terms,$main_terms);
					}
				}
...
if (!function_exists('wpqa_categories_select')) :
	function wpqa_categories_select($terms,$attr,$level) {
		...
		$show_option_none = apply_filters('wpqa_head_of_select_question_categories'.(isset($attr['parent_cat'])?'_subs':''),(isset($attr['show_option_none'])?$attr['show_option_none']:esc_html__('Select a Category','wpqa')));
		$exclude          = apply_filters('wpqa_exclude_question_category',array());
		...
		$select = wp_dropdown_categories(array_merge($exclude,
			array(
				...
				'echo'             => 0,
				'title_li'         => '',
				'class'            => 'form-control wpqa-custom-select cat-ajax '.$taxonomy.$class,
				'id'               => $class,
				'selected'         => $selected,
				'depth'            => 1,
...
function wpqa_add_edit_question($type,$popup = false,$user = false) {
	...
	if ($type == "edit") {
		...
		if ($user != "user") {
			$q_tag = "";
			if ($terms = wp_get_object_terms( $get_question, wpqa_question_tags )) :
				$terms_array = array();
				foreach ($terms as $term) :
			...
			$question_category = wp_get_post_terms($get_question,wpqa_question_categories,array("fields" => "ids"));
		}
	}
	...
	if ($user == "user") {
		...
		if (isset($display_name) && $display_name != "") {
			$out .= '<div class="ask-user-question">'.
				wpqa_get_user_avatar(array("user_id" => $get_user_id,"size" => 42,"class" => "rounded-circle")).
				sprintf(esc_html__("Ask %s a question","wpqa"),$display_name).
			'</div>';
		}
	}
	if ($type == "edit") {
		$return_url = wpqa_edit_permalink($get_question,wpqa_questions_type);
	}else {
	...
	'<form class="form-post wpqa_form" action="'.$return_url.'" method="post" enctype="multipart/form-data">'.(isset($_POST["form_type"]) && $_POST["form_type"] == $type."_question"?apply_filters('wpqa_'.($user == "user"?"user":$type).'_question',($user == "user"?"user":$type)):"").'
		<div class="form-inputs clearfix">';
			if ($type == "add" && !is_user_logged_in() && $ask_question_no_register == "on") {
				$out .= '<p>
					...
					<input name="username" id="question-username-'.$rand.'" class="form-control the-username" type="text" value="'.(isset($question_add['username'])?esc_attr($question_add['username']):'').'">
					<i class="icon-user"></i>
				...
				<p>
					...
					<input name="email" id="question-email-'.$rand.'" class="form-control the-email" type="text" value="'.(isset($question_add['email'])?esc_attr($question_add['email']):'').'">
					<i class="icon-mail"></i>
			...
			if ($type == "edit") {
				...
				$multicheck_poll        = get_post_meta($get_question,"multicheck_poll",true);
				...
				$term_category          = term_exists(esc_html($category_meta),wpqa_question_categories);
				if ($user != "user") {
			...
			if (isset($question_sort) && is_array($question_sort)) {
				foreach ($question_sort as $sort_key => $sort_value) {
					...
					if ($sort_key == "title_question" && ((isset($question_sort["title_question"]["value"]) && $question_sort["title_question"]["value"] == "title_question") || (isset($question_sort["comment_question"]["value"]) && $question_sort["comment_question"]["value"] != "comment_question"))) {
						...
						$out .= apply_filters('wpqa_add_edit_question_before_title',false,$type,$question_add,$question_edit,(isset($get_question)?$get_question:0)).
						($suggest_questions == "on"?'<div class="the-title-div">':'<p>').
							'<label for="question-title-'.$rand.'">'.esc_html__("Question Title","wpqa").'<span class="required">*</span></label>
							<input name="title" id="question-title-'.$rand.'" class="form-control the-title'.($suggest_questions == "on"?" suggest-questions live-search".(has_himer() || has_knowly()?"":" live-search-icon"):"").'" type="text" value="'.($type == "add" && isset($question_add['title'])?wp_unslash(esc_html($question_add['title'])):($type == "edit"?(isset($question_edit['title'])?wp_unslash(esc_html($question_edit['title'])):esc_html($get_post_q->post_title)):"")).'"'.apply_filters("wpqa_question_title_attrs",false).'>
							<i class="icon-chat"></i>
							'.($suggest_questions == "on"?'<div class="loader_2 search_loader"></div><div class="live-search-results mt-2 search-results results-empty"></div>':'').'
						...
						if ($category_single_multi == "ajax_2" && $type == "edit") {
							$question_category = wp_get_post_terms($get_question,wpqa_question_categories,array("fields" => "all"));
						}

						$category_value = "";
						if ($type == "add" && isset($question_add['category'])) {
							$category_value = $question_add['category'];
							if ($category_single_multi == "multi" && !is_array($category_value) && $category_value != "") {
								$category_value = array($category_value);
							}
						}else if ($type == "edit") {
							if (isset($question_edit['category'])) {
								$category_value = $question_edit['category'];
							}else if ($category_single_multi != "multi" && isset($question_category[0]->name)) {
								$category_value = $question_category[0]->name;
							}else if ($category_single_multi != "multi" && isset($category_meta) && $category_meta != "") {
								$category_value = $category_meta;
							}else if ($category_single_multi != "multi" && is_array($question_category) && isset($question_category[0])) {
							...
							}else if (is_tax(wpqa_question_categories)) {
						...
						$category_value = (!is_array($category_value)?esc_html($category_value):$category_value);
						$category_value = (is_array($category_value)?(empty($category_value)?"":$category_value):$category_value);

						if ($category_single_multi == "ajax_2") {
							$category_field = '<div class="p-category"><input name="category" id="question-category-'.$rand.'" class="form-control the-category-ajax" type="text" value="'.$category_value.'" autocomplete="off">
							<div class="loader_2 search_loader"></div>
							<div class="search-results results-empty"></div>
							</div>';
						}else {
							$category_field = wpqa_select_categories($rand,$category_value,null,($type == 'edit'?$get_question:''),wpqa_question_categories);
						}
						$category_area = '<div class="wpqa_category">
							<label for="question-category-'.$rand.'">'.esc_html__("Category","wpqa").($question_category_required == 'on'?'<span class="required">*</span>':'').'</label>
						...
						$out .= '<p class="wpqa_tag">
							...
							<input type="text" class="form-control input question_tags" name="'.wpqa_question_tags.'" id="question_tags-'.$rand.'" value="'.($type == "add" && isset($question_add[wpqa_question_tags])?wp_unslash(esc_html($question_add[wpqa_question_tags])):($type == "edit"?(isset($question_edit[wpqa_question_tags])?wp_unslash(esc_html($question_edit[wpqa_question_tags])):sanitize_text_field($q_tag)):"")).'" data-seperator=",">
						...
						if ($custom_poll_groups != "on" || ($custom_poll_groups == "on" && isset($user_login_group) && isset($poll_groups[$user_login_group]) && $poll_groups[$user_login_group] == $user_login_group)) {
							...
							if ($type == "add" && ((isset($question_add['multicheck_poll']) && ($question_add['multicheck_poll'] == "on" || $question_add['multicheck_poll'] == 1)) || (isset($add_question_default["multicheck_poll"]) && $add_question_default["multicheck_poll"] == "multicheck_poll" && empty($question_add)))) {
								$active_multicheck_poll = true;
							}else if ($type == "edit" && ((isset($question_edit['multicheck_poll']) && ($question_edit['multicheck_poll'] == "on" || $question_edit['multicheck_poll'] == 1) || (!isset($question_edit['multicheck_poll']) && ($multicheck_poll == "on" || $multicheck_poll == 1))))) {
								$active_multicheck_poll = true;
							}
							$poll_only = wpqa_options("poll_only");
							$multicheck_poll = wpqa_options("multicheck_poll");
							...
							if (($type == "add" && isset($question_add['question_image_poll']) && ($question_add['question_image_poll'] == "on" || $question_add['question_image_poll'] == 1)) || ($type == "edit" && ((isset($question_edit['question_image_poll']) && ($question_edit['question_image_poll'] == "on" || $question_edit['question_image_poll'] == 1) || (!isset($question_edit['question_image_poll']) && ($question_image_poll == "on" || $question_image_poll == 1)))))) {
								$checked_image = true;
							}
							
							$out .= '<div class="clearfix"></div>
							<div class="poll_options'.(isset($active_poll) || $poll_only == "on"?"":" wpqa_hide").($poll_only == "on"?" poll-only":" not-poll-only").($poll_image == "on"?" poll-image":" poll-not-image").($poll_image_title == "on"?" poll-title":" poll-not-title").(isset($checked_image)?" poll-activated-image":" poll-not-activated-image").'">';
								if ($multicheck_poll == "on") {
									$out .= '<p class="wpqa_checkbox_p wpqa_checkbox_poll">
										<label for="multicheck_poll-'.$rand.'">
											<span class="wpqa_checkbox"><input type="checkbox" id="multicheck_poll-'.$rand.'" class="multicheck_poll" value="on" name="multicheck_poll"'.(isset($active_multicheck_poll)?" checked='checked'":"").'></span>
											<span class="wpqa_checkbox_span">'.esc_html__("Is this poll is multicheck?","wpqa").'</span>
										</label>
									</p>';
								}
								if ($poll_image == "on") {
									$out .= '<p class="wpqa_checkbox_p">
										<label for="question_image_poll-'.$rand.'">
											<span class="wpqa_checkbox"><input type="checkbox" id="question_image_poll-'.$rand.'" class="question_image_poll" value="on" name="question_image_poll"'.(isset($checked_image)?" checked='checked'":"").'></span>
											<span class="wpqa_checkbox_span">'.esc_html__("Image poll?","wpqa").'</span>
										</label>
									</p>
									<div class="clearfix"></div>';
								}
								$out .= '<ul class="question_items question_polls_item list-unstyled sorting-area ui-sortable">';
									if ($type == "edit") {
											...
											$out .= '<li id="poll_li_'.(int)$ask['id'].'">';
												...
												$out .= '<div class="poll-li">';
													if (!isset($active_the_image) || $poll_image != 'on' || ($poll_image == 'on' && $poll_image_title == 'on')) {
														$out .= '<p>
															<input class="form-control ask" name="ask['.(int)$ask['id'].'][title]" value="'.(isset($ask['title']) && $ask['title'] != ""?wp_unslash(esc_html($ask['title'])):"").'" type="text">
								...
								$out .= '</ul>
								<button type="button" class="button-default-3 add_poll_button_js btn btn__primary btn__semi__height">'.esc_html__("Add More Answers","wpqa").'</button>
								<div class="clearfix"></div>
							</div>'.
						...
						if ($type == "edit") {
							...
							if ($added_file != "") {
								$out .= '<ul class="wpqa-delete-attachment"><li><a class="btn btn__primary btn__sm" href="'.wp_get_attachment_url($added_file).'"><i class="icon-attach"></i>'.esc_html__('Attachment','wpqa').'</a><a class="delete-this-attachment single-attachment btn btn__danger btn__sm" href="'.$added_file.'"><i class="icon-trash"></i>'.esc_html__('Delete','wpqa').'</a><div class="loader_2 loader_4"></div></li></ul>';
							}
							...
							if (isset($attachment_m) && is_array($attachment_m) && !empty($attachment_m)) {
								$out .= '<ul class="wpqa-delete-attachment">';
									foreach ($attachment_m as $key => $value) {
										$out .= '<li><a class="btn btn__primary btn__sm" href="'.wp_get_attachment_url($value["added_file"]).'"><i class="icon-attach"></i>'.esc_html__('Attachment','wpqa').'</a><a class="delete-this-attachment btn btn__danger btn__sm" data-id="'.$get_question.'" href="'.$value["added_file"].'"><i class="icon-trash"></i>'.esc_html__('Delete','wpqa').'</a><div class="loader_2 loader_4"></div></li>';
									}
								$out .= '</ul>';
							}
						}
						$out .= '<div class="question-multiple-upload question-upload-attachment">
							...
							<div class="clearfix"></div>
							<ul class="question_items question_upload_item list-unstyled sorting-area ui-sortable"></ul>
							<button type="button" class="button-default-3 add_upload_button_js btn btn__primary btn__semi__height">'.esc_html__("Add Field","wpqa").'</button>
							<div class="clearfix"></div>
						</div>';
						...
						if ($type == "edit") {
							...
							if ($_thumbnail_id != "") {
								...
								<div class="wpqa-delete-image d-flex align-items-center mb-4">
									...
									<div class="clearfix"></div>
									<div class="button-default wpqa-remove-image btn btn__danger btn__small__width" data-name="_thumbnail_id" data-type="post_meta" data-id="'.$get_question.'" data-image="'.$_thumbnail_id.'" data-nonce="'.wp_create_nonce("wpqa_remove_image").'">'.esc_html__("Delete","wpqa").'</div>
						...
						$out .= '<div class="wpqa_textarea wpqa_ask_question_textarea'.($editor_question_details == "on"?"":" wpqa_textarea_p").'">
							...
							$question_value = ($type == "add" && isset($question_add['comment'])?wpqa_esc_textarea($question_add['comment']):($type == "edit"?(isset($question_edit['comment'])?wpqa_esc_textarea($question_edit['comment']):$get_post_q->post_content):""));
							if ($editor_question_details == "on") {
								...
								ob_start();
								wp_editor($question_value,"question-details-".$type.'-'.$rand,$settings);
								$editor_contents = ob_get_clean();
								$out .= '<div class="the-details the-textarea">'.$editor_contents.'</div>';
							}else {
								$out .= '<textarea name="comment" id="question-details-'.$type.'-'.$rand.'" class="form-control the-textarea" aria-required="true" cols="58" rows="8"'.apply_filters("wpqa_question_content_attrs",false).'>'.$question_value.'</textarea>
						...
						<div class="video_description wpqa_hide"'.($type == "add" && isset($question_add['video_description']) && $question_add['video_description'] == "on"?" style='display:block;'":($type == "edit" && ((isset($question_edit['video_description']) && $question_edit['video_description'] == "on") || $q_video_description == "on")?" style='display:block;'":"")).'>
							<p>
								...
								<span class="styled-select">
									<select class="form-control" id="video_type-'.$rand.'" name="video_type">
										...
										<option value="tiktok"'.($type == "add" && isset($question_add['video_type']) && $question_add['video_type'] == "tiktok"?' selected="selected"':($type == "edit"?((isset($question_edit['video_type']) && $question_edit['video_type'] == "tiktok") || (isset($q_video_type) && $q_video_type == "tiktok")?' selected="selected"':''):'')).'>TikTok</option>
									</select>
								</span>
							...
							<p>
								...
								<input name="video_id" id="video_id-'.$rand.'" class="form-control video_id" type="text" value="'.esc_html(($type == "add" && isset($question_add['video_id'])?$question_add['video_id']:($type == "edit"?(isset($question_edit['video_id'])?$question_edit['video_id']:$q_video_id):""))).'">
								<i class="icon-play"></i>
		...
		<p class="form-submit mb-0">';
			...
			if (wpqa_is_add_questions()) {
				$get_user_id = apply_filters("wpqa_add_question_user",esc_html(get_query_var(apply_filters('wpqa_add_questions','add_question'))));
			}
			...
			$out .= '<input type="hidden" name="form_type" value="'.$type.'_question">'.apply_filters("wpqa_question_before_submit_button",false).'
			<input type="hidden" name="wpqa_'.$type.$user.'_question_nonce" value="'.wp_create_nonce("wpqa_".$type.$user."_question_nonce").'">
			'.(wpqa_input_button() == "button"?'<button type="submit" class="btn btn__primary btn__block btn__large__height button-default button-hide-click">'.($type == "add"?esc_html__("Publish Your Question","wpqa"):esc_html__("Submit","wpqa")).'</button>':'<input type="submit" value="'.($type == "add"?esc_html__("Publish Your Question","wpqa"):esc_html__("Submit","wpqa")).'" class="button-default button-hide-click">').'
...
/* Get question status */
function wpqa_get_question_status($question_id = 0,$user_id = 0) {
   			...
   			if ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type) {
	   			...
	   			if ($get_post->post_status == "draft") {
					...
					if ($get_current_user_id > 0) {
						wpqa_notifications_activities($get_current_user_id,"","","","","approved_question","activities","",wpqa_questions_type);
					}
					...
					$schedules_time_notification = wpqa_options("schedules_time_notification_questions");
					...
					if ($get_current_user_id > 0) {
						wpqa_notifications_activities($get_current_user_id,"","",$return,"","add_question","activities","",wpqa_questions_type);
					}
					wpqa_post_publish($get_post,(isset($not_user)?$not_user:0));
		...
		if (is_wp_error($return)) :
   			...
   			if ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type) {
   				...
   				if ($get_post->post_status == "draft") {
					wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("Your question was successfully added, It's under review.","wpqa").'</p></div>','wpqa_session');
					if ($user_id > 0) {
						wpqa_notifications_activities($user_id,"","","","","approved_question","activities","",wpqa_questions_type);
					}
					wp_redirect(esc_url(home_url('/')));
				}else {
					$get_question_user = get_post_meta($post_id,"user_id",true);
					...
					if ($post_author != $get_question_user && $get_question_user > 0) {
						wpqa_notifications_activities($get_question_user,$post_author,"",$post_id,"","add_question_user","notifications","",wpqa_questions_type);
					}
					if ($user_id > 0) {
						wpqa_notifications_activities($user_id,"","",$post_id,"","add_question","activities","",wpqa_questions_type);
					}
					wpqa_count_posts($post_type,$post_author,"+");
...
function wpqa_process_new_questions($data,$user = "") {
	...
	if ($form_type == "add_question") {
		...
		$fields = array(
			'title','comment','category','question_poll','multicheck_poll','question_image_poll','remember_answer',wpqa_question_tags,'video_type','video_id','video_description','sticky','ask','private_question','anonymously_question','attachment','attachment_m','featured_image','wpqa_captcha','username','email','terms_active','user_id'
		);
		...
		if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_id) && empty($roles["ask_question"])) || (!is_user_logged_in() && $ask_question != "on"))) {
			$errors->add('required','<strong>'.esc_html__("Error","wpqa").':&nbsp;</strong> '.esc_html__("Sorry, you do not have permission to ask a question.","wpqa"));
			if (!is_user_logged_in()) {
				$errors->add('required','<strong>'.esc_html__("Error","wpqa").':&nbsp;</strong> '.esc_html__("You must login to ask a question.","wpqa"));
			}
			...
			if (!is_user_logged_in() && $pay_ask == "on") {
				...
				if (!is_super_admin($user_id) && isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && ($custom_permission != "on" || ($custom_permission == "on" && empty($roles["ask_question_payment"])))) {
		...
		if (isset($question_sort["featured_image"]["value"]) && $question_sort["featured_image"]["value"] == "featured_image") {
			...
			if (isset($_FILES['featured_image']) && !empty($_FILES['featured_image']['name'])) :
				$types = array("image/jpeg","image/bmp","image/jpg","image/png","image/gif","image/tiff","image/ico");
				if (!isset($data['mobile']) && isset($_FILES['featured_image']['type']) && $_FILES['featured_image']['type'] != "" && !in_array($_FILES['featured_image']['type'],$types)) :
		...
		$data_post = array(
			'post_content' => ($editor_question_details == "on"?wpqa_esc_textarea($question_add['comment']):wpqa_esc_textarea($question_add['comment'])),
			'post_title'   => esc_html($question_title),
			...
			'post_type'	   => ((isset($question_add['user_id']) && $question_add['user_id'] != "") || $wpqa_add_question_user > 0?wpqa_asked_questions_type:wpqa_questions_type),
		);
		...
		if ((empty($question_add['user_id']) || (isset($question_add['user_id']) && $question_add['user_id'] == "")) && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question" && isset($question_add['category']) && $question_add['category']) {
			$category_single_multi = wpqa_options("category_single_multi");
			if ($category_single_multi == "ajax_2") {
				$term = term_exists(esc_html($question_add['category']),wpqa_question_categories);
				if ($term !== 0 && $term !== null) {
					wp_set_object_terms($question_id,esc_html($question_add['category']),wpqa_question_categories);
				}else {
				...
				if (is_array($question_add['category'])) {
					...
					$cat_ids = array();
					$cat_ids[] = get_term_by('id',(is_array($question_add['category'])?end($question_add['category']):$question_add['category']),wpqa_question_categories)->slug;
				}
				if (sizeof($cat_ids) > 0) :
					wp_set_object_terms($question_id,$cat_ids,wpqa_question_categories);
				endif;
			}
		}
		...
		if ($poll_only == "on" || (isset($question_add['question_poll']) && $question_add['question_poll'] != ""))  {
			update_post_meta($question_id,'question_poll','on');
			if ($question_add['multicheck_poll']) {
				update_post_meta($question_id,'multicheck_poll',esc_html($question_add['multicheck_poll']));
			}
		...
		if (isset($_FILES['ask']) && !empty($_FILES['ask'])) {
			...
			if (isset($files) && $files) {
				foreach ($files['name'] as $key => $value) {
					if ($files['name'][$key]) {
						$types = array("image/jpeg","image/bmp","image/jpg","image/png","image/gif","image/tiff","image/ico");
						if (!isset($data['mobile']) && isset($files['type'][$key]["image"]) && $files['type'][$key]["image"] != "" && !in_array($files['type'][$key]["image"],$types)) :
							$errors->add('upload-error',esc_html__("Attachment Error! Please upload image only.","wpqa"));
							return $errors;
						endif;
		...
		if (isset($question_add['ask']) && is_array($question_add['ask']) && !empty($question_add['ask'])) {
			foreach ($question_add['ask'] as $key => $value) {
				$question_add['ask'][$key]['title'] = wp_slash(esc_html($question_add['ask'][$key]['title']));
			}
		}
		...
		if ((empty($question_add['user_id']) || (isset($question_add['user_id']) && $question_add['user_id'] == "")) && isset($question_add[wpqa_question_tags])) :			
			$tags = explode(',',trim(stripslashes($question_add[wpqa_question_tags])));
			$tags = array_map('strtolower',$tags);
			$tags = array_map('trim',$tags);
			wp_set_object_terms($question_id,(sizeof($tags) > 0?$tags:array()),wpqa_question_tags);
		endif;
		...
		do_action(($user == "user"?"wpqa_finished_add_user_question":"wpqa_finished_add_question"),$question_id,$question_add,"add",wpqa_questions_type);
...
function wpqa_question($atts, $content = null) {
	...
	if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_id) && empty($roles["ask_question"])) || (!is_user_logged_in() && $ask_question != "on"))) {
		$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to ask a question","wpqa").(!is_user_logged_in()?", ".esc_html__("You must login to ask a question","wpqa"):'').'.'.' '.wpqa_paid_subscriptions().'</p></div>';
		if (!is_user_logged_in()) {
			if ($activate_login != "disabled") {
				$out .= do_shortcode("[wpqa_login".(isset($a["popup"]) && $a["popup"] == "popup" && (has_himer() || has_knowly())?" register='button'":"")."]").(isset($a["popup"]) && $a["popup"] == "popup"?"":($activate_register != "disabled"?"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>":""));
			}else {
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to ask a question","wpqa").'.</p></div>';
			}
		}
	}else if (!is_user_logged_in() && (($pay_ask != "on" && $ask_question_no_register != "on") || ($pay_ask == "on" && $unlogged_pay != "on"))) {
		if ($activate_login != "disabled") {
			$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("You must login to ask a question.","wpqa").'</p></div>'.do_shortcode("[wpqa_login".(isset($a["popup"]) && $a["popup"] == "popup" && (has_himer() || has_knowly())?" register='button'":"")."]").(isset($a["popup"]) && $a["popup"] == "popup"?"":($activate_register != "disabled"?"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>":""));
		}else {
			$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to ask a question","wpqa").'.</p></div>';
		}
		...
		if (!is_user_logged_in() && $pay_ask == "on" && $unlogged_pay != "on") {
			if ($activate_login != "disabled") {
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("You must login to ask a question.","wpqa").'</p></div>
				'.do_shortcode("[wpqa_login".(isset($a["popup"]) && $a["popup"] == "popup" && (has_himer() || has_knowly())?" register='button'":"")."]").(isset($a["popup"]) && $a["popup"] == "popup"?"":($activate_register != "disabled"?"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>":""));
			}else {
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to ask a question","wpqa").'.</p></div>';
			}
		}else {
			...
			if (!isset($display_name_ok)) {
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("You can't ask question to yourself.","wpqa").'</p></div>';
			}else if (!is_super_admin($user_id) && isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && ($custom_permission != "on" || ($custom_permission == "on" && empty($roles["ask_question_payment"])))) {
				...
				if ($ask_payment_style == "packages") {
					...
					$out .= '<a href="'.wpqa_checkout_link("ask_question",(isset($get_user_id) && $get_user_id > 0?$get_user_id:"")).'" class="button-default btn btn__primary" target="_blank">'.esc_html__("Pay to ask a question","wpqa").'</a>';
				}
			}else {
				$question_points_active = wpqa_options("question_points_active");
...
function wpqa_edit_question_attr() {
	...
	if (!is_user_logged_in()) {
		...
		if ($activate_login != "disabled") {
			...
			$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to edit question.","wpqa").'.</p></div>';
		}
	}else {
		...
		if ($edit_question == "on" || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
			...
			$post_type = (isset($get_question->post_type)?$get_question->post_type:"");
			if (isset($get_post) && $get_post != 0 && $get_question && ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type)) {
...
function wpqa_process_edit_questions($data,$user = "") {
	...
	if ($form_type == "edit_question") {
		...
		$fields = array(
			'title','comment','category','question_poll','multicheck_poll','question_image_poll','remember_answer',wpqa_question_tags,'video_type','video_id','video_description','sticky','ask','private_question','featured_image','publish_question','pending','attachment','attachment_m'
		);
		...
		$get_post_q = get_post($get_question);
		$post_type = $get_post_q->post_type;
		$user_id = get_current_user_id();
		$is_super_admin = is_super_admin($user_id);
		$anonymously_user = get_post_meta($get_post_q->ID,'anonymously_user',true);
		if (isset($get_question) && $get_question != 0 && $get_post_q && ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type)) {
			$moderators_permissions = wpqa_user_moderator($user_id);
		...
		if ((isset($question_sort["featured_image"]["value"]) && $question_sort["featured_image"]["value"] == "featured_image") || ($question_edit['question_poll'] && $question_edit['question_poll'] != "")) {
			require_once(ABSPATH . 'wp-admin/includes/media.php');
			require_once(ABSPATH . 'wp-admin/includes/image.php');
			require_once(ABSPATH . 'wp-admin/includes/file.php');
		}
		...
		if ($change_question_url == "on") {
			$post_name = array('post_name' => sanitize_text_field($question_edit['title']));
		}
		$moderators_permissions = wpqa_user_moderator($user_id);
		...
		if ($title_question === "on") {
			...
			$question_title = wpqa_excerpt_any($title_excerpt,sanitize_textarea_field($question_edit['comment']),'...',$title_excerpt_type);
		}
		
		$post_status = ($question_approved != "draft" || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit") || $is_super_admin || (isset($question_edit['publish_question']) && $question_edit['publish_question'] == "publish")?"publish":"draft");
		$data_post = array(
			'ID'           => (int)sanitize_text_field($question_id),
			'post_content' => ($editor_question_details == "on"?wpqa_esc_textarea($question_edit['comment']):wpqa_esc_textarea($question_edit['comment'])),
			'post_title'   => esc_html($question_title),
		...
		if (empty($get_question_user_id) && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question") {
			if (isset($question_edit['category']) && $question_edit['category']) {
				$category_single_multi = wpqa_options("category_single_multi");
				if ($category_single_multi == "ajax_2") {
					$term = term_exists(esc_html($question_edit['category']),wpqa_question_categories);
					if ($term !== 0 && $term !== null) {
						wp_set_object_terms($question_id,esc_html($question_edit['category']),wpqa_question_categories);
					}else {
					...
					if (is_array($question_edit['category'])) {
						...
						$cat_ids = array();
						$get_term_by = get_term_by('id',(is_array($question_edit['category'])?end($question_edit['category']):$question_edit['category']),wpqa_question_categories);
						$cat_ids[] = (isset($get_term_by->slug)?$get_term_by->slug:"");
					}
					if (sizeof($cat_ids) > 0) :
						wp_set_object_terms($question_id,$cat_ids,wpqa_question_categories);
					endif;
		...
		if ($question_edit['question_poll'] && $question_edit['question_poll'] != "")  {
			...
			if ($question_edit['multicheck_poll'] && $question_edit['multicheck_poll'] != "") {
				update_post_meta($question_id,'multicheck_poll',esc_html($question_edit['multicheck_poll']));
			}else {
				delete_post_meta($question_id,'multicheck_poll');
			}
					...
					if ($files['name'][$key]) {
						...
						if (!isset($attachment['error']) && $attachment) :
							$attachment_data = array(
								...
								'post_content'   => '',
								'post_status'	 => 'inherit',
								'post_author'    => ((!is_user_logged_in() && $ask_question_no_register == "on") || $anonymously_user > 0?0:$user_id),
							);
							$attachment_id = wp_insert_attachment($attachment_data,$attachment['file'],$question_id);
							$attachment_metadata = wp_generate_attachment_metadata($attachment_id,$attachment['file']);
							wp_update_attachment_metadata($attachment_id,$attachment_metadata);
			...
			if (isset($question_edit['ask']) && is_array($question_edit['ask']) && !empty($question_edit['ask'])) {
				foreach ($question_edit['ask'] as $key => $value) {
					$question_edit['ask'][$key]['title'] = wp_slash(esc_html($question_edit['ask'][$key]['title']));
				}
			}
			...
			if (isset($files) && $files) {
				...
				if (isset($attachment_m_array)) {
					...
					if (is_array($attachment_m) && !empty($attachment_m)) {
						update_post_meta($question_id,'attachment_m',array_merge($attachment_m,$attachment_m_array));
					}else {
						add_post_meta($question_id,'attachment_m',$attachment_m_array);
					}
				}
		...
		if (empty($get_question_user_id) && isset($question_edit[wpqa_question_tags])) :
			$tags = explode(',',trim(stripslashes($question_edit[wpqa_question_tags])));
			$tags = array_map('strtolower',$tags);
			$tags = array_map('trim',$tags);
			wp_set_object_terms($question_id,(sizeof($tags) > 0?$tags:array()),wpqa_question_tags);
		endif;
...
function wpqa_add_edit_question_errors($errors,$posted,$type,$question_sort,$get_question_user_id,$comment_question,$title_question) {
	...
	if ($type == "add" && $question_category_required == "on") {
		if ((empty($posted['user_id']) || (isset($posted['user_id']) && $posted['user_id'] == "")) && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question" && (empty($posted['category']) || $posted['category'] == '-1' || (is_array($posted['category']) && count($posted['category']) < 1 && (end($posted['category']) == "" || end($posted['category']) == "-1")))) {
			$errors->add('required-field','<strong>'.esc_html__("Error","wpqa").':&nbsp;</strong> '.esc_html__("There are required fields (category).","wpqa"));
		}
	}

	if ($type == "edit" && $question_category_required == "on") {
		if (empty($get_question_user_id) && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question" && (empty($posted['category']) || $posted['category'] == '-1' || (is_array($posted['category']) && count($posted['category']) < 1 && (end($posted['category']) == "" || end($posted['category']) == "-1")))) {
			$errors->add('required-field','<strong>'.esc_html__("Error","wpqa").':&nbsp;</strong> '.esc_html__("There are required fields (category).","wpqa"));
		}
	}
	...
	if (isset($question_sort["tags_question"]["value"]) && $question_sort["tags_question"]["value"] == "tags_question") {
		...
		if (!empty($posted[wpqa_question_tags])) {
			$tags = explode(',',trim(stripslashes($posted[wpqa_question_tags])));
			$tags = array_map('strtolower',$tags);
			$tags = array_map('trim',$tags);
		}else {
			$tags = array();
	...
	if (isset($question_sort["poll_question"]["value"]) && $question_sort["poll_question"]["value"] == "poll_question") {
		$poll_only = wpqa_options("poll_only");
		if ((empty($posted['user_id']) || (isset($posted['user_id']) && $posted['user_id'] == "")) && ($poll_only == "on" || (isset($posted['question_poll']) && $posted['question_poll'] == "on"))) {
			...
			if (isset($posted['ask']) && is_array($posted['ask'])) {
				foreach($posted['ask'] as $ask) {
					...
					$ask['title'] = (isset($ask['title'])?esc_html(trim($ask['title'])):"");
