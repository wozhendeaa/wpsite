...
function wpqa_add_category_attr($atts, $content = null) {
	...
	if ($allow_user_to_add_category == "on") {
		...
		if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_get_current_user_id) && empty($roles["add_category"])) || (!is_user_logged_in() && $add_category != "on"))) {
			$out = '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to add category.","wpqa").' '.wpqa_paid_subscriptions().'</p></div>';
...
function wpqa_process_add_category() {
	...
	if ($form_type == "add_category") {
		...
		$term = term_exists(esc_html($category_add['category']),wpqa_question_categories);
		if (0 !== $term && null !== $term) {
		...
		/* Successful */
		return wp_unslash(esc_html($category_add['category']));
	}
}
/* Add category request */
function wpqa_add_category_request($category,$post_id = '') {
	$user_id = get_current_user_id();
	wpqa_new_request($user_id,"request_category",esc_html($category),$post_id,"request_category");
}
/* Add category form */
	...
	$out = '<form class="form-post wpqa_form" method="post">'.(isset($_POST["form_type"]) && $_POST["form_type"] == "add_category"?apply_filters('wpqa_add_category','category'):"").'
		<div class="form-inputs clearfix">';
			$out .= '<p>
				...
				<input class="form-control" type="text" name="category" id="category-name-'.$rand.'" value="'.(isset($category_add['category'])?wp_unslash(esc_html($category_add['category'])):"").'">
				<i class="icon-chat"></i>
		...
		<p class="form-submit mb-0">
			...
			'.(wpqa_input_button() == "button"?'<button type="submit" class="btn btn__primary btn__block btn__large__height button-default button-hide-click">'.esc_html__("Add a new category","wpqa").'</button>':'<input type="submit" value="'.esc_html__("Add a new category","wpqa").'" class="button-default button-hide-click">').'
