...
function wpqa_insert_payment($response,$user_id,$_coupon = '',$_coupon_value = '') {
	...
	$latest_charge = (isset($response['latest_charge']) && $response['latest_charge'] != ""?$response['latest_charge']:"");
	$payment_method = (isset($response['payment']) && $response['payment'] != ""?$response['payment']:"");
	$insert_data = array(
		'post_title'   => $response["item_name"],
		'post_content' => $item_transaction.($latest_charge != "" && $latest_charge != $item_transaction?" - ".$latest_charge:""),
		'post_status'  => "publish",
		'post_author'  => $user_id,
		'post_type'    => 'statement'
	);
	...
	if ($post_id == 0 || is_wp_error($post_id)) {
		...
		if ($latest_charge != "") {
			update_post_meta($post_id,"payment_latest_charge",$latest_charge);
		}
		$variables = array(
			"payment_new" => 1,
		);
...
function wpqa_get_money_note($views) {
	$payment_convert = get_option("wpqa_payment_convert");
	if ($payment_convert == true) {
		...
		if (is_array($items) && !empty($items)) {
			...
			update_option("wpqa_payment_convert",true);
		}
	}
...
function wpqa_statement_columns($old_columns){
	...
	$columns["author_s"]    = esc_html__("Author","wpqa");
	$columns["date_s"]      = esc_html__("Date","wpqa");
	$columns["transaction"] = esc_html__("Transaction","wpqa");
	$columns["email"]       = esc_html__("Payer email","wpqa")." - (".esc_html__("Tesing","wpqa").")";
	return $columns;
}
...
function wpqa_statements_status($status) {
	...
	$payment_type_group    = wpqa_options('payment_type_group');
	...
	$get_status = (isset($_GET['item_number'])?esc_html($_GET['item_number']):'');
	$get_points = (isset($_GET['buy'])?esc_html($_GET['buy']):'');
	...
	if ($pay_group == "on" && $group_payment_style == "once") {
		$status['add_group'] = '<a href="'.admin_url('edit.php?post_type=statement&item_number=add_group').'"'.($get_status == "add_group"?' class="current"':'').'>'.esc_html__('Create a new group','wpqa').' ('.wpqa_meta_count("payment_item_number","add_group").')</a>';
	}
	if ($pay_group == "on" && $group_payment_style == "packages") {
		$status['buy_groups'] = '<a href="'.admin_url('edit.php?post_type=statement&item_number=buy_groups').'"'.($get_status == "buy_groups"?' class="current"':'').'>'.esc_html__('Buy groups','wpqa').' ('.wpqa_meta_count("payment_item_number","buy_groups").')</a>';
	}
...
function wpqa_statements_exclude($screen) {
	if ($screen->id != 'edit-statement')
		return;
	$get_status = (isset($_GET['item_number'])?esc_html($_GET['item_number']):'');
	$get_payment = (isset($_GET['payment_methods'])?esc_html($_GET['payment_methods']):'');
	$get_points = (isset($_GET['buy'])?esc_html($_GET['buy']):'');
	$statement = (isset($_GET['statement'])?esc_html($_GET['statement']):'');
	if ($get_status == "ask_question" || $get_status == "buy_questions" || $get_status == "add_group" || $get_status == "buy_groups" || $get_status == "add_post" || $get_status == "buy_posts" || $get_status == "pay_sticky" || $get_status == "subscribe" || $get_status == "buy_points" || $get_status == "pay_answer") {
...
function wpqa_payment_subscribe_filter() {
	...
	if ($post_type == 'statement') {
		$subscriptions_payment = wpqa_options("subscriptions_payment");
		if ($subscriptions_payment == "on") {
			$payment_subscribe = (isset($_GET['payment_subscribe'])?esc_html($_GET['payment_subscribe']):'');
