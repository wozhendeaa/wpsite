...
/* Make the groups with number */
add_filter('post_type_link','wpqa_group_number_slug',10,2);
function wpqa_group_number_slug($post_link,$post) {
	$group_slug_numbers = wpqa_options("group_slug_numbers");
	if ($group_slug_numbers == "on") {
		if ('group' != $post->post_type || 'publish' != $post->post_status) {
			return $post_link;
		}
		$group_slug = wpqa_options('group_slug');
		$group_slug = ($group_slug != ""?$group_slug:"group");
		$post_link = str_replace('/'.$group_slug.'/'.$post->post_name,'/'.$group_slug.'/'.$post->ID,$post_link);
	}
	return $post_link;
}
/* Remove group slug */
add_filter('post_type_link','wpqa_group_remove_slug',10,2);
function wpqa_group_remove_slug($post_link,$post) {
	$remove_group_slug = wpqa_options("remove_group_slug");
	if ($remove_group_slug == "on") {
		if ('group' != $post->post_type || 'publish' != $post->post_status) {
			return $post_link;
		}
		$group_slug = wpqa_options('group_slug');
		$group_slug = ($group_slug != ""?$group_slug:"group");
		$post_link = str_replace('/'.$group_slug.'/','/',$post_link);
	}
	return $post_link;
}
		...
		if ($active_groups == "on") {
			...
			register_post_type('group',
				array(
					...
					'hierarchical'        => false,
					'rewrite'             => array('slug' => apply_filters("wpqa_group_slug",($remove_group_slug == "on"?false:$group_slug)),'hierarchical' => true,'with_front' => false),
					'query_var'           => true,
					'show_in_rest'        => true,
			...
			$group_slug_numbers = wpqa_options("group_slug_numbers");
			if ($group_slug_numbers == "on") {
				$removed = ($remove_group_slug == "on"?'':$group_slug.'/');
				add_rewrite_rule($removed.'([0-9]+)?$','index.php?post_type='.$group_slug.'&p=$matches[1]','top');
			}
		}
	}
endif;
