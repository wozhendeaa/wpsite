	...
	switch ( $column ) {
		case 'content_n' :
			...
			if (!empty($notification_result["post_id"])) {
				$get_post = get_post($notification_result["post_id"]);
			...
			if (!empty($notification_result["comment_id"])) {
				...
				if (!empty($get_comment)) {
					echo '<a class="tooltip_s" data-title="'.(isset($get_post_type) && ($get_post_type == wpqa_questions_type || $get_post_type == wpqa_asked_questions_type)?esc_html__("View answer","wpqa"):esc_html__("View comment","wpqa")).'" href="'.get_comment_link($notification_result["comment_id"]).'"><i class="dashicons dashicons-admin-comments"></i></a>';
				}
				...
				if ($get_post_status != "trash" && !empty($get_the_permalink)) {
					echo '<a class="tooltip_s" data-title="'.$view_post.'" href="'.$get_the_permalink.'"><i class="dashicons dashicons-editor-help"></i></a>';
				}
			}
		break;
		case 'author_n' :
...
if (!function_exists('wpqa_get_notifications')) :
	function wpqa_get_notifications($user_id,$item_number,$more_button,$count = false,$more_button_ul = false,$ul_class = '') {
		$output = '';
		if ($count == true) {
		...
		if ($notifications_query->have_posts()) {
			...
			if ($more_button == "on" && $more_button_ul == true) {
				$output .= "<li class='notifications__item d-flex align-items-center justify-content-center notification__show-all'><a href='".esc_url(wpqa_get_profile_permalink($user_id,"notifications"))."'>".esc_html__("Show all notifications.","wpqa")."</a></li>";
			}
			$output .= "</ul>";
			...
			$output .= "<li class='notifications__item d-flex align-items-center justify-content-center'><div class='notification__body'>".esc_html__("There are no notifications yet.","wpqa")."</div></li></ul>";
		}
		if ($more_button_ul == false) {
			$output .= '</div>';
		}
...
function wpqa_notification_activity_result($post,$type = "notification",$admin = "") {
	...
	$new = get_post_meta($post->ID,$type."_new",true);
...
if (!function_exists('wpqa_show_notifications')) :
	function wpqa_show_notifications($notification_array,$show_date = "",$show_icon = "on") {
		$output = "";
		if ($show_date == "on") {
			$class = (isset($notification_array["text"])?preg_replace('/[^a-zA-Z0-9._\-]/','',strtolower($notification_array["text"])):"");
			$output .= "<li class='notifications__item d-flex".(isset($notification_array["another_user_id"])?" ".wpqa_get_gender_class($notification_array["another_user_id"],(isset($notification_array["post_id"])?$notification_array["post_id"]:0),(isset($notification_array["comment_id"])?$notification_array["comment_id"]:0)):'').($class != ""?" notifications__".$class:"")."'>";
		}
		...
		if ($show_icon == "on") {
			...
			if ($result_icon != "") {
			...
			}else if ($notification_array["text"] == "new_badge") {
				$output .= "<i class='icon-trophy'></i>";
				...
				$output .= "<i class='icon-plus'></i>";
			}else if ($notification_array["text"] == "question_reaction_like" || $notification_array["text"] == "answer_reaction_like" || $notification_array["text"] == "posts_reaction_like" || $notification_array["text"] == "comment_reaction_like" || $notification_array["text"] == "question_remove_reaction" || $notification_array["text"] == "answer_remove_reaction" || $notification_array["text"] == "posts_remove_reaction" || $notification_array["text"] == "comment_remove_reaction") {
				$output .= "<i class='icon-thumbsup'></i>";
			}else if ($notification_array["text"] == "question_reaction_love" || $notification_array["text"] == "answer_reaction_love" || $notification_array["text"] == "posts_reaction_love" || $notification_array["text"] == "comment_reaction_love") {
				$output .= "<i class='fa-solid fa-heart'></i>";
			}else if ($notification_array["text"] == "question_reaction_hug" || $notification_array["text"] == "answer_reaction_hug" || $notification_array["text"] == "posts_reaction_hug" || $notification_array["text"] == "comment_reaction_hug") {
				$output .= "<i class='fa-solid fa-face-kiss-wink-heart'></i>";
			}else if ($notification_array["text"] == "question_reaction_haha" || $notification_array["text"] == "answer_reaction_haha" || $notification_array["text"] == "posts_reaction_haha" || $notification_array["text"] == "comment_reaction_haha") {
				$output .= "<i class='fa-solid fa-face-laugh-squint'></i>";
			}else if ($notification_array["text"] == "question_reaction_wow" || $notification_array["text"] == "answer_reaction_wow" || $notification_array["text"] == "posts_reaction_wow" || $notification_array["text"] == "comment_reaction_wow") {
				$output .= "<i class='fa-solid fa-face-surprise'></i>";
			}else if ($notification_array["text"] == "question_reaction_sad" || $notification_array["text"] == "answer_reaction_sad" || $notification_array["text"] == "posts_reaction_sad" || $notification_array["text"] == "comment_reaction_sad") {
				$output .= "<i class='fa-solid fa-face-frown-open'></i>";
			}else if ($notification_array["text"] == "question_reaction_angry" || $notification_array["text"] == "answer_reaction_angry" || $notification_array["text"] == "posts_reaction_angry" || $notification_array["text"] == "comment_reaction_angry") {
			...
			}else if (!empty($notification_array["post_id"]) || $notification_array["text"] == "cronjob_question" || $notification_array["text"] == "cronjob_post" || $notification_array["text"] == "cronjob_questions" || $notification_array["text"] == "cronjob_posts") {
			...
			if ((((isset($notification_array["comment_id"]) && $notification_array["comment_id"] != "") || $notification_array["text"] == "add_question_user" || $notification_array["text"] == "add_question" || $notification_array["text"] == "add_post" || $notification_array["text"] == "poll_question") && (empty($notification_array["username"]) || $notification_array["username"] == "unlogged") && isset($notification_array["another_user_id"]) && $notification_array["another_user_id"] == 0) || (!empty($notification_array["another_user_id"]) || !empty($notification_array["username"])) && $notification_array["text"] != "admin_add_points" && $notification_array["text"] != "admin_remove_points") {
				if ((((isset($notification_array["comment_id"]) && $notification_array["comment_id"] != "") || $notification_array["text"] == "add_question_user" || $notification_array["text"] == "add_question" || $notification_array["text"] == "add_post" || $notification_array["text"] == "poll_question") && isset($notification_array["another_user_id"]) && $notification_array["another_user_id"] == 0) || (isset($display_name) && $display_name != "")) {
					...
					if (!empty($notification_array["username"]) && $notification_array["username"] != "unlogged") {
						$output .= esc_html($notification_array["username"])." ";
					}
					if (((isset($notification_array["comment_id"]) && $notification_array["comment_id"] != "") || $notification_array["text"] == "add_question_user" || $notification_array["text"] == "add_question" || $notification_array["text"] == "add_post") && empty($notification_array["username"]) && isset($notification_array["another_user_id"]) && $notification_array["another_user_id"] == 0) {
		...
		if (!empty($notification_array["post_id"]) && !empty($notification_array["comment_id"]) && $get_post_status != "trash" && isset($get_comment) && $get_comment->comment_approved != "spam" && $get_comment->comment_approved != "trash") {
			$output .= '<a class="notification__question notification__question-dark" href="'.esc_url($get_the_permalink.(isset($notification_array["comment_id"])?"#comment-".$notification_array["comment_id"]:"")).'">';
		}
		if (!empty($notification_array["post_id"]) && empty($notification_array["comment_id"]) && $get_post_status != "trash" && isset($get_the_permalink) && $get_the_permalink != "") {
			if ($notification_array["text"] == "comment_reaction_like" || $notification_array["text"] == "comment_reaction_haha" || $notification_array["text"] == "comment_reaction_hug" || $notification_array["text"] == "comment_reaction_sad" || $notification_array["text"] == "comment_reaction_wow" || $notification_array["text"] == "comment_reaction_angry" || $notification_array["text"] == "comment_reaction_love" || $notification_array["text"] == "comment_remove_reaction") {
				$get_post = get_post($notification_array["post_id"]);
				if (isset($get_post->post_type) && $get_post->post_type == "posts") {
					$get_the_permalink = wpqa_custom_permalink($notification_array["post_id"],"view_posts_group","view_group_post");
				}
			}
			if ($notification_array["text"] == "posts_like" || $notification_array["text"] == "approved_posts" || $notification_array["text"] == "posts_reaction_like" || $notification_array["text"] == "posts_reaction_haha" || $notification_array["text"] == "posts_reaction_hug" || $notification_array["text"] == "posts_reaction_sad" || $notification_array["text"] == "posts_reaction_wow" || $notification_array["text"] == "posts_reaction_angry" || $notification_array["text"] == "posts_reaction_love" || $notification_array["text"] == "posts_remove_reaction") {
				$get_the_permalink = wpqa_custom_permalink($notification_array["post_id"],"view_posts_group","view_group_post");
			}
			$output .= '<a class="notification__question notification__question-dark" href="'.esc_url($get_the_permalink).'">';
		}
		if ($notification_array["text"] == "cronjob_question" || $notification_array["text"] == "cronjob_post" || $notification_array["text"] == "cronjob_questions" || $notification_array["text"] == "cronjob_posts") {
			if ($notification_array["text"] == "cronjob_questions" || $notification_array["text"] == "cronjob_question") {
				$cronjob_type = "question";
			}else {
				$cronjob_type = "post";
			}
			if ($notification_array["text"] == "cronjob_questions" || $notification_array["text"] == "cronjob_posts") {
				$notifications_posts_link = wpqa_options("notifications_".$cronjob_type."s_link");
				if ($notifications_posts_link == "custom_page") {
					$notifications_posts_custom_page = wpqa_options("notifications_".$cronjob_type."s_custom_page");
					if ($notifications_posts_custom_page > 0) {
						$last_url = get_the_permalink($notifications_posts_custom_page);
					}
				}else if ($notifications_posts_link == "custom_link") {
					$notifications_posts_custom_link = wpqa_options("notifications_".$cronjob_type."s_custom_link");
					if ($notifications_posts_custom_link != "") {
						$last_url = $notifications_posts_custom_link;
					}
				}
			}
			$last_url = (isset($get_the_permalink) && $get_the_permalink != ""?$get_the_permalink:(isset($last_url) && $last_url != ""?$last_url:home_url('/')));
			if ((isset($get_post_status) && $get_post_status != "trash" && isset($get_the_permalink) && $get_the_permalink != "") || (!isset($get_the_permalink) && isset($last_url) && $last_url != "")) {
				$output .= '<a class="notification__question notification__question-dark" href="'.esc_url($last_url).'">';
			}
		}
			...
			if ($result_text != "") {
				...
				$wpqa_badge = (isset($notification_array["more_text"]) && $notification_array["more_text"] != ""?$notification_array["more_text"]:"");
				if ($wpqa_badge != "") {
					$wpqa_category = explode("_wpqa_categories_",$wpqa_badge);
					$category = (isset($wpqa_category[1]) && $wpqa_category[1] > 0?$wpqa_category[1]:0);
					if ($category > 0) {
						$get_term = get_term($category,wpqa_question_categories);
					}
					$wpqa_badge = explode("wpqa_new_badge_",$wpqa_category[0]);
					$badge = (isset($wpqa_badge[1]) && $wpqa_badge[1] != ""?$wpqa_badge[1]:"");
					$output .= sprintf(esc_html__("Woohoo! You've earned the %s badge%s.","wpqa"),"\"<span>".$badge."</span>\"",($category > 0 && isset($get_term) && isset($get_term->slug)?" ".esc_html__("in the","wpqa")." <a href='".get_term_link($get_term)."'>".$get_term->name."</a>"." ".esc_html__("category","wpqa"):""));
				}else {
					$output .= esc_html__("You've earned new badge.","wpqa");
				}
			}else if ($notification_array["text"] == "accepted_category") {
				$output .= esc_html__("The administrator approved your category.","wpqa");
			}else if ($notification_array["text"] == "cronjob_questions") {
				$output .= esc_html__("Added new questions.","wpqa");
			}else if ($notification_array["text"] == "cronjob_posts") {
				$output .= esc_html__("Added new posts.","wpqa");
			}else if ($notification_array["text"] == "cronjob_question") {
				$output .= esc_html__("Added a new question.","wpqa");
			}else if ($notification_array["text"] == "cronjob_post") {
				...
				$output .= esc_html__("added a new question.","wpqa");
			}else if ($notification_array["text"] == "add_post") {
				$output .= esc_html__("added a new post.","wpqa");
				...
				$output .= "<a class='notification__question notification__question-dark' href='".esc_url(wpqa_get_profile_permalink(get_current_user_id(),"messages"))."'>".esc_html__("The administrator approved your message.","wpqa")."</a>";
				...
				$output .= sprintf(esc_html__("The administrator %s your %s.","wpqa"),$notification_array["more_text"],(isset($notification_array["type_of_item"]) && ($notification_array["type_of_item"] == wpqa_questions_type || $notification_array["type_of_item"] == wpqa_asked_questions_type)?esc_html__("question","wpqa"):esc_html__("post","wpqa")));
				...
				if (isset($notification_array["type_of_item"]) && ($notification_array["type_of_item"] == wpqa_questions_type || $notification_array["type_of_item"] == wpqa_asked_questions_type)) {
					if (!empty($notification_array["another_user_id"])) {
				...
				$output .= "<a class='notification__question notification__question-dark' href='".esc_url(wpqa_get_profile_permalink(get_current_user_id(),"messages"))."'>".esc_html__("sent a message for you.","wpqa")."</a>";
			...
			}else if ($notification_array["text"] == "question_reaction_like" || $notification_array["text"] == "question_reaction_love" || $notification_array["text"] == "question_reaction_hug" || $notification_array["text"] == "question_reaction_haha" || $notification_array["text"] == "question_reaction_wow" || $notification_array["text"] == "question_reaction_sad" || $notification_array["text"] == "question_reaction_angry") {
				$output .= esc_html__("reacted to your question.","wpqa");
			}else if ($notification_array["text"] == "answer_reaction_like" || $notification_array["text"] == "answer_reaction_love" || $notification_array["text"] == "answer_reaction_hug" || $notification_array["text"] == "answer_reaction_haha" || $notification_array["text"] == "answer_reaction_wow" || $notification_array["text"] == "answer_reaction_sad" || $notification_array["text"] == "answer_reaction_angry") {
				$output .= esc_html__("reacted to your answer.","wpqa");
			}else if ($notification_array["text"] == "posts_reaction_like" || $notification_array["text"] == "posts_reaction_love" || $notification_array["text"] == "posts_reaction_hug" || $notification_array["text"] == "posts_reaction_haha" || $notification_array["text"] == "posts_reaction_wow" || $notification_array["text"] == "posts_reaction_sad" || $notification_array["text"] == "posts_reaction_angry") {
				$output .= esc_html__("reacted to your group post.","wpqa");
			}else if ($notification_array["text"] == "comment_reaction_like" || $notification_array["text"] == "comment_reaction_love" || $notification_array["text"] == "comment_reaction_hug" || $notification_array["text"] == "comment_reaction_haha" || $notification_array["text"] == "comment_reaction_wow" || $notification_array["text"] == "comment_reaction_sad" || $notification_array["text"] == "comment_reaction_angry") {
				$output .= esc_html__("reacted to your comment.","wpqa");
			}else {
				$output .= $notification_array["text"];
			}
			if ($notification_array["text"] == "cronjob_question" || $notification_array["text"] == "cronjob_post" || $notification_array["text"] == "cronjob_questions" || $notification_array["text"] == "cronjob_posts" || (!empty($notification_array["post_id"]) && !empty($notification_array["comment_id"]) && $get_post_status != "trash" && isset($get_comment) && $get_comment->comment_approved != "spam" && $get_comment->comment_approved != "trash") || (!empty($notification_array["post_id"]) && empty($notification_array["comment_id"]) && $get_post_status != "trash" && isset($get_the_permalink) && $get_the_permalink != "")) {
			$output .= '</a>';
		}
		...
		if ($notification_array["text"] != "new_badge" && !empty($notification_array["more_text"]) && $notification_array["more_text"] != "admin_add_points" && $notification_array["text"] != "action_post" && $notification_array["text"] != "action_comment" && $notification_array["text"] != "delete_reason") {
			$output .= " - ".esc_html($notification_array["more_text"]).".";
		}
		
		if ($show_date == "on") {
			$output .= "<span class='notifications-date notification__date d-block mt-2'>".$notification_array["time"]."</span>
			</div></li>";
		}
		return $output;	
	}
