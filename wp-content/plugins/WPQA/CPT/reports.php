...
function wpqa_get_report_note($views) {
	...
	foreach ($types as $type) {
		...
		if ($wpqa_option > 0) {
			...
			if (is_array($wpqa_option_array)) {
				foreach ($wpqa_option_array as $key => $value) {
					...
					if ($type == "_answer") {
						...
						$wpqa_profile_url = wpqa_profile_url($wpqa_one_option[$value]["user_id"]);
						$display_name = get_the_author_meta('display_name',$wpqa_one_option[$value]["user_id"]);
			...
			for ($reports = 1; $reports <= $count; $reports++) {
				...
				}else if (isset($wpqa_one_option["user_id"])) {
					$wpqa_profile_url = wpqa_profile_url($wpqa_one_option["user_id"]);
					$display_name = get_the_author_meta('display_name',$wpqa_one_option["user_id"]);
					if ($wpqa_profile_url != "" && $display_name != "") {
						$available_post = true;
					}
				}else {
				...
				if (isset($available_post) && !isset($wpqa_one_option["moved_done"])) {
					...
					if ($post_id == 0 || is_wp_error($post_id)) {
						...
						$variables = array("post_id","report_new","the_author","comment_id","report_type","user_id");
						foreach ($variables as $value) {
							if ($value == "report_type") {
								...
								update_post_meta($post_id,$value,$type);
							...
							}else if ($value == "user_id" && isset($wpqa_one_option[$value]) && $wpqa_one_option[$value] != "") {
								update_post_meta($post_id,"report_user_id",$wpqa_one_option[$value]);
							}
						}
					}
					sleep(1);
	...
	update_option("new_answer_reports",0);
	update_option("new_user_reports",0);
	do_action("wpqa_action_after_all_reports");
	return $views;
}
	...
	switch ( $column ) {
		case 'link' :
			if ($report_type == "answer") {
				...
				echo '<a target="_blank" href="'.esc_url(wpqa_profile_url(get_post_meta($post->ID,"report_user_id",true))).'">'.esc_html__("View user","wpqa").'</a>';
			}else {
		...
		case 'type' :
			...
			if ($report_type == "answer") {
				$report_type = esc_html__("Answer","wpqa");
			}else if ($report_type == "user") {
				$report_type = esc_html__("User","wpqa");
			}else {
				$report_type = esc_html__("Question","wpqa");
			}
			echo '<span class="money-span'.($report_new == 1?" report_new":"").'">'.$report_type.'</span>';
		...
		case 'author_r' :
			...
			if ($the_author != "") {
				if ($the_author == 1) {
					...
					echo esc_html($the_author);
				}
			}else {
				$user_name = get_the_author_meta('display_name',$post->post_author);
				if ($user_name != "") {
					echo '<a target="_blank" href="'.wpqa_profile_url((int)$post->post_author).'"><strong>'.$user_name.'</strong></a><a class="tooltip_s" data-title="'.esc_html__("View reports","wpqa").'" href="'.admin_url('edit.php?post_type=report&author='.$post->post_author).'"><i class="dashicons dashicons-flag"></i></a>';
				}else {
...
function wpqa_add_admin_report() {
	...
	if ($report_users == "on") {
		add_submenu_page('edit.php?post_type=report',esc_html__('Users','wpqa'),esc_html__('Users','wpqa'),'manage_options','edit.php?post_type=report&types=users');
	}
}
...
if (!function_exists('wpqa_reports_status')) :
	function wpqa_reports_status($status) {
		...
		$report_users_array = array();
		$report_users = wpqa_options("report_users");
		if ($report_users == "on") {
			$report_users_array = array(
				'users' => '<a href="'.admin_url('edit.php?post_type=report&types=users').'"'.($get_status == "users"?' class="current"':'').'>'.esc_html__('Users reports','wpqa').' ('.wpqa_meta_count("report_type","user").')</a>'
			);
		}
...
if (!function_exists('wpqa_reports_exclude')) :
	function wpqa_reports_exclude($screen) {
		if ($screen->id != 'edit-report')
			return;
		$get_status = (isset($_GET['types'])?esc_html($_GET['types']):'');
		if ($get_status == "questions" || $get_status == "answers" || $get_status == "users") {
...
if (!function_exists('wpqa_list_reports')) :
	function wpqa_list_reports($clauses) {
		...
		if ($get_status == "questions") {
			...
			$clauses->query_vars['post_type'] = "report";
		}else if ($get_status == "users") {
			$clauses->query_vars['meta_key'] = "report_type";
			$clauses->query_vars['meta_value'] = "user";
			$clauses->query_vars['post_type'] = "report";
		}
	}
endif;
...
function wpqa_report_filter() {
	global $post_type;
	if ($post_type == 'report') {
		$report_type = (isset($_GET['report_type'])?esc_html($_GET['report_type']):'');
		echo '<select name="report_type">
			...
			if ($report_users == "on") {
				$key = "user";
				echo '<option '.selected($report_type,$key,false).' value="'.$key.'">'.esc_html__('Users reports','wpqa').'</option>';
			}
		echo '</select>';
