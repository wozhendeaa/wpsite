	...
	function wpqa_get_payment_coupons($user_id,$item_id = 0,$days_sticky = 0,$kind_of_payment = "ask_question",$points_number = 0,$price = 0,$package_name = "",$message = "",$number_vars = 1,$found_key = "") {
		...
		if ($payment_by_points == "points" || $payment_by_points == "payments_points") {
			...
			if ($kind_of_payment == "sticky") {
				...
				$points_message = '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("Please pay by points to allow to be able to add an answer with %s.","wpqa"),$points_price." ".esc_html__("points","wpqa")).'</p></div>';
			}else if ($kind_of_payment == "subscribe") {
				...
				$points_message = '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("Please pay by points to create a group with %s.","wpqa"),$points_price." ".esc_html__("points","wpqa")).'</p></div>';
			}else if ($kind_of_payment == "buy_groups") {
				$points_price = (int)(isset($packages_payment[$found_key]) && isset($packages_payment[$found_key]["package_points"])?$packages_payment[$found_key]["package_points"]:0);
				$points_message = '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("Please pay by points to create %s with %s.","wpqa"),sprintf(_n("%s group","%s groups",$item_id,"wpqa"),$item_id),' '.$points_price.' '.esc_html__("points","wpqa")).'</p></div>';
			}else if ($kind_of_payment == "add_post") {
		...
		if ($payment_by_points == "points" || !isset($payment_methods_activated)) {
			...
			if ($user_id > 0) {
				...
				if ($points_price <= $points_user) {
					$output .= wpqa_process_points_form($return_url,$item_process,$points_price,($kind_of_payment != "buy_questions" && $kind_of_payment != "buy_groups" && $kind_of_payment != "buy_posts"?($item_id > 0?$item_id:0):""),"",($kind_of_payment != "buy_questions" && $kind_of_payment != "buy_groups" && $kind_of_payment != "buy_posts"?$item_id:""),($item_id != ""?$item_id:""));
				}else {
					...
					if (count($multi_currencies) > 1) {
						$select_currencies = '<form class="wpqa_form multi_currencies_form" method="post">
							...
							if (isset($select_currencies) && $select_currencies != "") {
								...
								}else if ($kind_of_payment == "subscribe" || strpos($kind_of_payment,"pay-subscribe-module") !== false) {
			...
			if ($last_payment > 0 && isset($payment_methods_activated)) {
				if ($kind_of_payment == "subscribe" || strpos($kind_of_payment,"pay-subscribe-module") !== false) {
							...
							foreach ($payment_methods as $key => $value) {
								...
								if ($key == "points" && $payment_methods["points"]["value"] == "points") {
									...
									$output .= apply_filters("wpqa_payment_note_form",false,$kind_of_payment).'<form method="post" action="'.$paypal_url.'">';
										if (($kind_of_payment == "subscribe" || strpos($kind_of_payment,"pay-subscribe-module") !== false) && $plan_name != "lifetime") {
