...
function wpqa_pay_to_answer_link($return,$user_id,$custom_permission,$roles,$post_id) {
	...
	if (!is_super_admin($user_id) && (($pay_to_answer != "paid" && $_allow_to_answer > 0) || $_allow_to_answer < 1) && ($custom_permission != "on" || ($custom_permission == "on" && empty($roles["add_answer_payment"])))) {
		$return = '<div class="card widget"><div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("Please make a payment to be able to add an answer.","wpqa").'</p></div>
		<a href="'.wpqa_checkout_link("answer",($post_id > 0?$post_id:"")).'" class="button-default btn btn__primary button-pay-answer" target="_blank">'.esc_html__("Pay to add an answer","wpqa").'</a></div>';
	}
	return $return;
}
