...
function wpqa_payment_succeeded($user_id,$response) {
	...
	if (($payment_item_transaction != "" && $item_transaction != $payment_item_transaction) || (isset($_POST["process"]) && $_POST["process"] == "subscribe" && isset($_POST["package_subscribe"]) && $_POST["package_subscribe"] != "") || (isset($response['reward']) && $response['reward'] != "") || (isset($response['trial']) && $response['trial'] != "")) {
		...
		if (!isset($have_posts)) {
			...
			$another_way_payment_filter = apply_filters("wpqa_another_way_payment_filter",true,array("user_id" => $user_id,"item_transaction" => $payment_item_transaction,"item_price" => $response['item_price'],"item_name" => $response['item_no'],"item_currency" => $response['item_currency'],"payer_email" => $payer_email,"first_name" => $first_name,"last_name" => $last_name,"custom" => $custom,"str_replace" => $str_replace,"customer" => (isset($response['customer'])?$response['customer']:""),"subscr_id" => (isset($response['subscr_id'])?$response['subscr_id']:"")));
			if ($another_way_payment_filter == true) {
				...
				if (!isset($response["renew"])) {
					...
					if ($payment_success != "" && isset($response['payment']) && $response['payment'] == "PayPal") {
...
function wpqa_not_enough_points($session = "session") {
	...
	if ($session == "session") {
		wpqa_session('<div class="alert-message error"><i class="icon-cancel"></i><p>'.$message.'</p></div>','wpqa_session');
	}else {
		return $message;
	}
}
