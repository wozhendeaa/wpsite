		...
		if ($search_value != "") {
			...
			<section<?php do_action("wpqa_search_section_tag",$search_type)?>>
				<?php if ( isset($wpqa_query) && $wpqa_query->have_posts() ) :
					...
					<div class="post-articles<?php do_action("wpqa_post_articles_tag",$search_type);echo (isset($its_post_type) && ($its_post_type == wpqa_questions_type || $its_post_type == wpqa_asked_questions_type)?" question-articles".(isset($question_columns) && $question_columns == "style_2"?" row row-warp".(isset($masonry_style) && $masonry_style == "on"?" isotope":""):""):"").(isset($its_post_type) && $its_post_type  == wpqa_knowledgebase_type?" knowledgebase-articles":"").($post_pagination == "none"?" no-pagination":"").(empty($more_link)?" articles-no-pagination":"")?>">
						<?php while ($wpqa_query->have_posts()) : $wpqa_query->the_post();
							...
							if ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type) {
								...
								$loop_page = "knowledgebase";
							}
							if ($filter_search_type == true) {
								do_action("wpqa_search_include_loop",$search_type);
							}else {
								include locate_template("includes/".$loop_page."-setting.php");
