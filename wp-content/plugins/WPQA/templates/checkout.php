	...
	if ($payment_available == true) {
		...
		if ($unlogged_pay == "on" || is_user_logged_in() || $logged == true) {
			...
			if ($checkout_value != "") {
				...
				if (isset($show_allow_group_message)) {
					...
					if ($_allow_to_group > 0) {
						echo '<div class="alert-message warning"><i class="icon-flag"></i><p>'.sprintf(esc_html__("You can already create %s you can pay more if you want.","wpqa"),sprintf(_n("%s group","%s groups",$_allow_to_group,"wpqa"),$_allow_to_group)).'</p></div>';
					}
				}
				...
				if (isset($allow_to_continue) && $allow_to_continue == true && $price > 0) {
					...
					echo wpqa_get_payment_coupons($user_id,$item_id,(isset($days_sticky)?$days_sticky:0),$checkout_value,$points_number,$price,$package_name,$custom_message,$number_vars,(isset($found_key)?$found_key:""));
				}else {
			...
			if ($activate_login != "disabled") {
				...
				echo '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to pay.","wpqa").'.</p></div>';
			}
		}
	}else {
