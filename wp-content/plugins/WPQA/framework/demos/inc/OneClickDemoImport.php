...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
	...
	protected function __construct() {
		// Actions.
		add_action( 'admin_menu', array( $this, 'create_plugin_page' ), 16 );
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_html( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'wpqa' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'wpqa' ),
						'dialog_no'             => esc_html__( 'Cancel', 'wpqa' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'wpqa' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'wpqa' ),
					),
					'dialog_options' => apply_filters( 'pt-ocdi/confirmation_dialog_options', array() )
				)
			);
			if (is_rtl()) {
				wp_enqueue_style( 'ocdi-main-css', PT_OCDI_URL . 'assets/css/main-rtl.css', array() , PT_OCDI_VERSION );
			}else {
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'wpqa' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'wpqa' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'wpqa' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'wpqa' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			if ( ! apply_filters( 'pt-ocdi/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome!', 'wpqa' );

				$response['message'] .= sprintf(
					__( '%1$sWasn\'t this a great One Click Demo Import experience?%2$s', 'wpqa' ),
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', 'wpqa' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'wpqa' ),
