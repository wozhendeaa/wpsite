...
if (!function_exists('wpqa_paid_subscriptions')) :
	function wpqa_paid_subscriptions($show = '') {
		...
		if ($subscriptions_payment == "on") {
			$out = ($show == true?'<div class="pop-footer pop-footer-subscriptions">':'').'<a class="subscriptions-link" href="'.wpqa_subscriptions_permalink().'" target="_blank">'.esc_html__("Please subscribe to paid membership","wpqa").'<i class="icon-sound"></i></a>'.($show == true?'</div>':'');
			return $out;
		}
	}
endif;
...
/* Reward subscription */
add_filter("wpqa_action_before_user_stats","wpqa_reward_subscription");
			...
			if ($subscriptions_payment == "on" && $reward_subscription == "on" && wpqa_is_user_owner()) {
				...
				if ($plan_name != "") {
					...
					if (isset($request_subscription)) {
						if (is_super_admin($user_id)) {
							$out .= '<div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("You are admin, so you can't subscribe.","wpqa").'</p></div>';
						}else {
