...
if (!function_exists('wpqa_action_user_avatar')) :
	function wpqa_action_user_avatar($args = array()) {
		if (isset($args["size"]) && isset($args["user_id"])) {
			echo wpqa_get_user_avatar(array("user_id" => $args["user_id"],"size" => $args["size"],"user_name" => (isset($args["name"])?$args["name"]:""),"user" => (isset($args["user"])?$args["user"]:""),"post" => (isset($args["post"])?$args["post"]:""),"comment" => (isset($args["comment"])?$args["comment"]:""),"email" => (isset($args["email"])?$args["email"]:""),"class" => (isset($args["class"])?$args["class"]:"")));
		}
	}
endif;
...
if (!function_exists('wpqa_action_avatar_link')) :
	function wpqa_action_avatar_link($args = array()) {
		if (isset($args["size"]) && isset($args["user_id"])) {
			echo wpqa_get_avatar_link(array("user_id" => $args["user_id"],"size" => $args["size"],"user_name" => (isset($args["name"])?$args["name"]:""),"user" => (isset($args["user"])?$args["user"]:""),"pop" => (isset($args["pop"])?$args["pop"]:""),"span" => (isset($args["span"])?$args["span"]:""),"post" => (isset($args["post"])?$args["post"]:""),"comment" => (isset($args["comment"])?$args["comment"]:""),"email" => (isset($args["email"])?$args["email"]:""),"class" => (isset($args["class"])?$args["class"]:"")));
		}
	}
endif;
/* Get the user avatar */
...
if (!function_exists('wpqa_get_user_avatar_url')) :
	function wpqa_get_user_avatar_url($your_avatar,$size,$user_name) {
		...
		if (is_numeric($your_avatar) && $your_avatar > 0 && $avatar_num == true) {
...
if (!function_exists('wpqa_get_user_avatar')) :
	function wpqa_get_user_avatar($args = array()) {
		...
		$avatar = "<img class='avatar avatar-".$size.($class != ""?" ".$class:"")." photo' alt='".esc_attr($user_name)."' title='".esc_attr($user_name)."' width='".$size."' height='".$size."' srcset='".$image." 1x, ".$image_2." 2x' src='".$image."'>";
		return $avatar;
	}
endif;
...
if (!function_exists('wpqa_get_avatar_link')) :
	function wpqa_get_avatar_link($args = array()) {
		...
		$avatar = wpqa_get_user_avatar(array("user_id" => $user_id,"size" => $size,"user_name" => $user_name,"user" => $user,"post" => $post,"comment" => $comment,"email" => $email,"class" => $class));
		
		$out = ($span == 'span'?'<div class="author-image author__avatar author-image-'.$size.'">':'');
			if ($user_id > 0) {
				...
				if ($pop == "pop" && $author_image_pop == "on") {
					$out .= '<div class="author-image-pop-2 member-card" data-user="'.$user_id.'"><div class="author-pop-loader"><div class="loader_2"></div></div></div>';
				}
			}else {
				if (!empty($comment)) {
