			...
			if (get_option('permalink_structure')) {
				...
				if ($profile_slug_numbers != "on") {
					...
					if ($profile_type == "login") {
						$user_name = trim(urldecode(esc_html(wpqa_get_user_login($wpqa_user_id))));
					}else {
						$user_name = trim(urldecode(esc_html(wpqa_get_user_nicename($wpqa_user_id))));
					}
					$user_name_2 = str_ireplace("-"," ",$user_name);
				}
				if (($profile_slug_numbers != "on" && $user_name != $wpqa_user && $user_name_2 != $wpqa_user) || (isset($_GET['wpqa_user_id']) && $_GET['wpqa_user_id'] > 0 && $_GET['wpqa_user_id'] == $wpqa_user_id)) {
...
if (!function_exists('wpqa_rewrite_rules')) :
	function wpqa_rewrite_rules() {
		...
		$user_blocking_slug       = wpqa_options('blocking_slug');
		...
		$user_joined_groups_slug  = wpqa_options('joined_groups_slug');
		$user_managed_groups_slug = wpqa_options('managed_groups_slug');
		...
		$user_blocking_id         = apply_filters('wpqa_blocking','blocking');
		...
		$blocking_rule                 = $user_rule . $user_blocking_slug       . '/?$';
		$blocking_page_rule            = $user_rule . $user_blocking_slug       . $page_rules;
		...
		$joined_groups_page_rule       = $user_rule . $user_joined_groups_slug  . $page_rules;
		$managed_groups_rule           = $user_rule . $user_managed_groups_slug . '/?$';
		$managed_groups_page_rule      = $user_rule . $user_managed_groups_slug . $page_rules;
		...
		$user_pages = array(
			...
			$blocking_rule            => $user_blocking_id,
		...
		$user_paged_pages = array(
			...
			$blocking_page_rule            => $user_blocking_id,
...
if (!function_exists('wpqa_parse_query')) :
	function wpqa_parse_query($posts_query) {
		...
		if ( !empty( $is_user ) ) {
			...
			$is_blocking            = $posts_query->get(apply_filters('wpqa_blocking','blocking'));
			...
			$is_joined_groups       = $posts_query->get(apply_filters('wpqa_joined_groups','joined_groups'));
			$is_managed_groups      = $posts_query->get(apply_filters('wpqa_managed_groups','managed_groups'));
...
function wpqa_add_question_permalink($user = '',$user_id = '') {
	...
	if ($ask_question_to_users == "on" && ((wpqa_is_user_profile() && $user == "user") || ($user == "user" && $user_id != ""))) {
		...
		$profile_type = wpqa_options("profile_type");
		if ($profile_type == "login") {
			$user_name = esc_html(urlencode(trim(wpqa_get_user_login($wpqa_user_id))));
		}else {
			$user_name = esc_html(urlencode(trim(wpqa_get_user_nicename($wpqa_user_id))));
		}
		...
		if (get_option('permalink_structure')) {
			...
			if ($profile_slug_numbers == 'on') {
				...
				if ( empty( $user_nicename ) || $profile_type == 'nicename' ) {
					$user_nicename = esc_html(urldecode(urlencode(trim(wpqa_get_user_nicename($user_id)))));
				}else {
					$user_nicename = esc_html(urldecode(urlencode(trim(wpqa_get_user_login($user_id)))));
				}
