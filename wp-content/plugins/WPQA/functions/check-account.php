...
if (!function_exists('wpqa_check_user_account')) :
	function wpqa_check_user_account($return = false,$confirm = false) {
		...
		if ($active_referral == "on" && !is_user_logged_in() && isset($_GET['invite']) && $_GET['invite'] != "") {
		...
		}else if ((is_user_logged_in() && ((isset($if_user_id->caps["activation"]) && $if_user_id->caps["activation"] == 1) || $edit_email != "")) || (!is_user_logged_in() && isset($_GET['u']) && isset($_GET['activate']))) {
			...
			if (isset($_GET['u']) && isset($_GET['activate'])) {
				...
				if ($activation == $get_activate) {
					if ($edit_email != "") {
						if (email_exists($edit_email)) {
							wpqa_session('<div class="alert-message warning alert-confirm-email"><i class="icon-flag"></i><p>'.sprintf(esc_html__('This email is already registered, please choose another one, kindly %1$s Edit your profile %2$s to change your email.','wpqa'),'<a href="'.esc_url(wpqa_get_profile_permalink($user_id,"edit")).'">','</a>').'</p></div>','wpqa_session');
						}else {
							$args = array(
								'ID'         => $user_id,
							...
							$send_text = wpqa_send_mail(
								array(
									'content' => wpqa_options("edited_email_link"),
									'user_id' => $user_id
								)
							);
							$email_title = wpqa_options("title_edited_email_link");
							$email_title = ($email_title != ""?$email_title:esc_html__("Edited email","wpqa"));
							...
							$wpqa_session = esc_html__("Your new email edited.","wpqa");
						}
					}else {
						...
						if ($activate_review_users == true || $user_review == "on") {
							...
							$wpqa_session = esc_html__("Your membership was activated, But it need a review first, You will be notified via mail when it has been approved.","wpqa");
						}else {
							update_user_meta($user_id,"user_activated","activated");
							do_action("wpqa_user_activated",$user_id);
							$wpqa_session = esc_html__("Your membership was activated.","wpqa");
						}
					}
					if (isset($wpqa_session)) {
						wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.$wpqa_session.'</p></div>','wpqa_session');
					}
					wp_safe_redirect(esc_url(home_url('/')));
					exit();
				}else {
					if ($return == true) {
						if (is_user_logged_in()) {
							echo '<div class="alert-message warning alert-confirm-email"><i class="icon-flag"></i><p>'.sprintf(esc_html__('Your confirmation link has expired or not valid, Please request a new link, kindly %1$s Click here %2$s to re-send another confirmation mail.','wpqa'),'<a href="'.esc_url_raw(add_query_arg(array('get_activate' => 'do','edit' => ($edit_email != ""?true:false)),esc_url(home_url('/')))).'">','</a>').'</p></div>';
						}else {
				...
				if (isset($_GET['get_activate']) && $_GET['get_activate'] == "do") {
					wpqa_resend_confirmation($user_id,(isset($_GET['edit']) && $_GET['edit'] == true?"edit":""));
					wpqa_session('<div class="alert-message success alert-confirm-email"><i class="icon-check"></i><p>'.esc_html__("Check your mail again.","wpqa").'</p></div>','wpqa_session');
					wp_safe_redirect(esc_url(home_url('/')));
					die();
				}else {
					if ($return == true) {
...
/* Resend confirmation */
function wpqa_resend_confirmation($user_id,$edit = "") {
	if ($edit == "edit") {
