...
function wpqa_custom_widget_search($form) {
	...
	$form = '<form role="search" method="get" class="search-form main-search-form" action="'.esc_url(wpqa_get_search_permalink()).'">
		<label>
			<input type="search" class="form-control search-field'.($live_search == "on"?" live-search":"").'"'.($live_search == "on"?" autocomplete='off'":"").' placeholder="'.esc_attr__("Search ...","wpqa").'" value="'.wpqa_search_terms().'" name="search">';
			if ($live_search == "on") {
			...
			$form .= '<select name="search_type" class="search_type">';
				...
				if (isset($search_attrs) && is_array($search_attrs) && !empty($search_attrs)) {
					foreach ($search_attrs as $key => $value) {
						...
						}else if ($key == wpqa_question_categories && isset($search_attrs[wpqa_question_categories]["value"]) && $search_attrs[wpqa_question_categories]["value"] == wpqa_question_categories) {
							$form .= '<option '.selected((isset($search_type) && $search_type != ""?$search_type:""),wpqa_question_categories,false).' value='.wpqa_question_categories.'>'.esc_html__("Question categories","wpqa").'</option>';
						}else if ($key == wpqa_question_tags && isset($search_attrs[wpqa_question_tags]["value"]) && $search_attrs[wpqa_question_tags]["value"] == wpqa_question_tags) {
							$form .= '<option '.selected((isset($search_type) && $search_type != ""?$search_type:""),wpqa_question_tags,false).' value='.wpqa_question_tags.'>'.esc_html__("Question tags","wpqa").'</option>';
						}else if ($key == "knowledgebases" && isset($search_attrs["knowledgebases"]["value"]) && $search_attrs["knowledgebases"]["value"] == "knowledgebases") {
							$form .= '<option '.selected((isset($search_type) && $search_type != ""?$search_type:""),"knowledgebases",false).' value="knowledgebases">'.esc_html__("Knowledgebases","wpqa").'</option>';
						}else if ($key == wpqa_knowledgebase_categories && isset($search_attrs[wpqa_knowledgebase_categories]["value"]) && $search_attrs[wpqa_knowledgebase_categories]["value"] == wpqa_knowledgebase_categories) {
							$form .= '<option '.selected((isset($search_type) && $search_type != ""?$search_type:""),wpqa_knowledgebase_categories,false).' value='.wpqa_knowledgebase_categories.'>'.esc_html__("Knowledgebase categories","wpqa").'</option>';
						}else if ($key == wpqa_knowledgebase_tags && isset($search_attrs[wpqa_knowledgebase_tags]["value"]) && $search_attrs[wpqa_knowledgebase_tags]["value"] == wpqa_knowledgebase_tags) {
							$form .= '<option '.selected((isset($search_type) && $search_type != ""?$search_type:""),wpqa_knowledgebase_tags,false).' value='.wpqa_knowledgebase_tags.'>'.esc_html__("Knowledgebase tags","wpqa").'</option>';
		...
		'.(wpqa_input_button() == "button"?'<button type="submit" class="search-submit btn btn__primary">'.esc_attr__("Search","wpqa").'</button>':'<input type="submit" class="search-submit" value="'.esc_attr__("Search","wpqa").'">').'
	</form>';
	return $form;
}
...
if (!function_exists('wpqa_get_countries')) :
	function wpqa_get_countries() {
		$countries = array(
			...
			'AX' => esc_html__( 'Aland Islands', 'wpqa' ),
