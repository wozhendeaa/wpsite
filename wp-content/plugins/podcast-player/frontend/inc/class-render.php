...
class Render {
	...
	public function search_field( $classname ) {
		...
		// Depricated:: This is required for all display styles.
		// if ( Validation_Fn::is_single_episode_layout( count( $this->items ), $this->args['display-style'], $single_display ) ) {
		// 	return '';
		// }

		return $this->get_template( 'list', 'search-field' );
	}
	...
	public function episodes_list() {
		...
		foreach ( $items as $key => $item ) {
			...
			if ( 'modern' === $style ) {
				// TODO: Modern style episode link should cover full area for better click UX.
