...
class Feed extends Singleton {
	...
	public function display_podcast( $args ) {
		...
		if ( is_wp_error( $args['url'] ) ) {
			...
			$this->display_error_message( $args['url'] );
			return;
		}
		...
		if ( is_wp_error( $feed ) ) {
			$this->display_error_message( $feed );
			return;
		}
	...
	private function get_required_data_fields( $args ) {
		$default_fields = array( 'title', 'date', 'link', 'src', 'mediatype', 'duration' );
		$conditional    = array(
	...
	private function prepare_feed_items( $items, $counter, $items_count, $args = array() ) {
		...
		return apply_filters( 'podcast_player_feed_items', $feed_items, 'feed' );
	}
	...
	public function fetch_episodes() {
		...
		list(
			$loaded, $displayed, $max_items, $instance, $sortby, $filterby, $fixed, $args, $lotsize, $term
		) = $val;
	...
	public function search_episodes() {
		 ...
		 */
		list( $r, $i ) = $this->get_search_results( $items, $term, $displayed );
	 ...
	 */
	private function get_search_results( $items, $term, $displayed ) {
		...
		foreach ( $items as $key => $item ) {
			...
			if ( false !== strpos( $item_title, $term ) ) {
				if ( $i > $displayed ) {
					$results[ $key ] = $item;
				}
				unset( $items[ $key ] );
