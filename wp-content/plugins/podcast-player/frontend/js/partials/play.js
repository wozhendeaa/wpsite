...
class PlayEpisode {
	...
	updateCookie() {
		...
		if ( this.playTime ) {
			this.mediaObj.media.setCurrentTime( this.playTime );
			this.playTime = false;
		}
	...
	modNextBtn() {
		...
		if (this.listItem && !this.listItem.hasClass('episode-list__search-entry')) {
			const newItem = this.listItem.next('.episode-list__entry');
			if (!newItem.length) {
				// TODO: Do not load new episodes while checking next episdoes availability.
				// Just check if new episodes are available. Load new episodes only if
				// Load more OR play next button is clicked.
