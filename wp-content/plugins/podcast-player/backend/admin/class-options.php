...
class Options {
	...
	public function __construct() {
		...
		$this->modules = array(
			...
			'help'     => array(
				'label' => esc_html__( 'FAQ', 'podcast-player' ),
			),
		);
	...
	public static function init() {
		...
		add_action( 'wp_ajax_pp_feed_editor', array( $inst, 'feed_editor' ) );
		add_action( 'wp_ajax_nopriv_pp_feed_editor', array( $inst, 'feed_editor' ) );
	}
	...
	public function get_setting_fields() {
		return apply_filters(
			...
			array(
				...
				// 	'id'          => 'update_method',
				// 	'description' => esc_html__( 'Default updates are very efficient. However, if that\'s not working for you due to caching plugin, you can use cron update method.', 'podcast-player' ),
				// 	'link'        => '',
				// 	'type'        => 'checkbox',
				// 	'default'     => '',
				// 	'section'     => 'general',
	...
	public function feed_editor() {
		...
		$topt   = get_transient( $time );
		$text   = 'refresh' === $type ? 'refreshed' : $type;

		if ( false === $option ) {
			$output = array(
				'error' => esc_html__( 'Feed data not available. Either data already ', 'podcast-player' ) . esc_html( $text ) . esc_html__( ' OR feed url is not correct.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
		...
		switch ( $type ) {
			case 'refresh':
				...
				if ( is_wp_error( $feed ) ) {
					$message = '<p><strong>' . esc_html__( 'RSS Error:', 'podcast-player' ) . '</strong> ' . esc_html( $feed->get_error_message() ) . '</p>';
				} else {
